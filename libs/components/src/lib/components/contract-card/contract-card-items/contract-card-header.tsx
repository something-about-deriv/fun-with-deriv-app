...
const ContractCardHeader = ({
  ...
  const current_tick = contract_info.tick_count
    ? getCurrentTick(contract_info)
    : null;
  const {
    growth_rate,
    underlying,
  ...
  const displayed_trade_param =
    contract_type_list_info.find(
      (contract_type_item_info) => contract_type_item_info.is_param_displayed
    )?.displayed_param || '';
