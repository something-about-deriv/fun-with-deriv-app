...
type TAccumulatorCardBody = {
  ...
  connectWithContractUpdate?: React.ComponentProps<
    typeof ToggleCardDialog
  ...
  getContractById: React.ComponentProps<
    typeof ToggleCardDialog
  >['getContractById'];
  indicative?: number;
  is_sold: boolean;
...
const AccumulatorCardBody = ({
  ...
  const { CURRENT_STAKE, INITIAL_STAKE, TAKE_PROFIT, TOTAL_PROFIT_LOSS } =
    getCardLabels();
  let is_won, is_loss;
  if (profit) {
    is_won = +profit > 0;
