...
import { TContractInfo, TContractStore } from '@deriv/shared';
...
export type TGeneralContractCardBodyProps = {
  ...
  connectWithContractUpdate?: (
    contract_update_form: React.ElementType
  ) => React.ElementType;
  currency: string;
...
const ContractUpdateForm = (props: TContractUpdateFormProps) => {
  ...
  const { stop_loss, take_profit } = getLimitOrderAmount(
  ...
  const error_messages: Record<string, string[] | undefined> = {
    take_profit: has_contract_update_take_profit
      ? take_profit_error
      : undefined,
    stop_loss: has_contract_update_stop_loss ? stop_loss_error : undefined,
  };
  const has_validation_errors = Object.keys(error_messages).some(
    (field) => error_messages[field]?.length
  );
  ...
  const is_valid_contract_update = is_multiplier
    ? is_valid_multiplier_contract_update
    : !!is_take_profit_valid;

  const getStateToCompare = (
    _state: Partial<
      ReturnType<typeof getContractUpdateConfig> & TContractUpdateFormProps
    >
  ) => {
    const props_to_pick = [
      'has_contract_update_take_profit',
  ...
  return (
    <React.Fragment>
      ...
      >
        <div className="dc-contract-card-dialog__input">
          {take_profit_input}
        </div>
        {is_multiplier && (
          <div className="dc-contract-card-dialog__input">
