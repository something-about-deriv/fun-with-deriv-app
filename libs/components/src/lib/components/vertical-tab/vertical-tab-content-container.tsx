...
const SideNotes = ({ class_name, side_notes }: TSideNotes) => {
  return (
    <div
      className={classNames('dc-vertical-tab__content-side-note', {
        [class_name as string]: !!side_notes,
      })}
...
const ContentWrapper = ({
  children,
  has_side_note,
}: React.PropsWithChildren<TContentWrapper>) => {
  if (has_side_note) {
...
const Content = ({ is_routed, items, selected }: TContent) => {
  ...
  const [side_notes, setSideNotes] = React.useState<React.ReactNode[] | null>(
    null
  );
