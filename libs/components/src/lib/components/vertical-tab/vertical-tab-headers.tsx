...
const offsetTop = (
  ...
  if (selected_el) {
    ...
    calculated_offset =
      selected_el.offsetTop && selected_el.offsetTop - item_offset;
  }
...
const VerticalTabHeaders = ({
  ...
  React.useEffect(() => {
    const selected_item = items.find(
      (item) =>
        item[selectedKey as keyof TItem] ===
        selected[selectedKey as keyof TItem]
    );
    if (selected_item?.label)
      setTop(
        offsetTop(extra_offset, is_floating, ref, {
  ...
  return (
    ...
    >
      {!is_floating && header_title && (
        <VerticalTabHeaderTitle header_title={header_title} />
      )}
