...
const getFilteredItems = (
  ...
  if (should_filter_by_char) {
    return list.filter((item) =>
      typeof item === 'string'
        ? matchStringByChar(item, val)
        : matchStringByChar(item.text || '', val)
    );
  }
  return list.filter((item) =>
    typeof item === 'string'
...
const Autocomplete = React.memo((props: TAutocompleteProps) => {
  ...
  useBlockScroll(
    list_portal_id && should_show_list ? input_wrapper_ref : undefined
  );
    ...
    if (has_updating_list) {
      ...
      if (is_list_visible) {
        if (typeof onSearch === 'function') {
          ...
          new_filtered_items = getFilteredItems(
            value.toLowerCase(),
            list_items
          );
        }
      ...
      if (historyValue) {
        const index = new_filtered_items.findIndex(
          (object) =>
            (typeof object === 'object' ? object.text : object) === historyValue
        );
  ...
  const setActiveUp = () => {
    if (active_index !== -1) {
      ...
      if (should_scroll_to_last) {
        ...
        dropdown_ref.current?.scrollTo({
          top: list_height,
          behavior: 'smooth',
        });
      } else {
        const item_height = Number(
          list_item_ref.current?.getBoundingClientRect().height
        );
        const item_top =
          Math.floor(
            Number(list_item_ref.current?.getBoundingClientRect()?.top)
          ) - item_height;

        if (!isListItemWithinView(item_top)) {
          const top_of_list =
            Number(list_item_ref.current?.offsetTop) - item_height;
          dropdown_ref.current?.scrollTo({
            top: top_of_list,
            behavior: 'smooth',
          });
  ...
  const isListItemWithinView = (item_top: number) => {
    ...
    const wrapper_top = Math.floor(
      Number(list_wrapper_ref.current?.getBoundingClientRect().top)
    );
    const wrapper_bottom =
      Math.floor(
        Number(list_wrapper_ref.current?.getBoundingClientRect().top)
      ) + list_height;
    ...
    if (active_index === -1 || !list_item_ref.current) {
      ...
      if (should_scroll_to_first) {
        ...
        const item_height =
          list_item_ref.current.getBoundingClientRect().height;
        const item_top =
          Math.floor(list_item_ref.current.getBoundingClientRect().top) +
          item_height +
          item_height / 2;
        const list_height = Number(dropdown_ref.current?.clientHeight);
        ...
        if (!isListItemWithinView(item_top)) {
          ...
          const bottom_of_list =
            list_item_ref.current.offsetTop - items_above * item_height;
          dropdown_ref.current?.scrollTo?.({
            top: bottom_of_list,
            behavior: 'smooth',
          });
  ...
  return (
    <div
      ...
      className={classNames('dc-autocomplete', className)}
    >
      ...
      <DropdownList
        ...
        style={{
          width: input_wrapper_ref.current
            ? `${input_wrapper_ref.current.offsetWidth}px`
            : '100%',
          ...(style || {
