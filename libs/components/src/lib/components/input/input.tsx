...
const InputWrapper = ({
  children,
  has_footer,
}: React.PropsWithChildren<TInputWrapper>) =>
  has_footer ? (
    <div className="dc-input__wrapper">{children}</div>
  ) : (
...
>(
  (
    ...
    const has_footer =
      !!has_character_counter || (!!hint && !!is_relative_hint);
      ...
      <InputWrapper has_footer={has_footer}>
          ...
          >
            ...
            {props.type === 'textarea' ? (
              <textarea
                ...
                // TODO: gotta fix this fast, please check this out, I have no idea what's going on here
            ...
            {label && (
              <label
                className={classNames('dc-input__label', label_className)}
                htmlFor={props.id}
              >
                {label}
              </label>
          ...
          <div>
            {!has_footer && (
              <React.Fragment>
                ...
                {warn && (
                  <Field className={classNameWarn} message={warn} type="warn" />
                )}
        ...
        {has_footer && (
          ...
          <div className="dc-input__footer">
            {error && (
              <Field className={classNameError} message={error} type="error" />
            )}
            {warn && (
              <Field className={classNameWarn} message={warn} type="warn" />
            )}
