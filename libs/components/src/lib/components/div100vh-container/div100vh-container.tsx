...
const Div100vhContainer = ({
  ...
  const height_rule = height_offset
    ? `calc(100rvh - ${height_offset})`
    : 'calc(100rvh)';
  const height_style = {
    height: max_autoheight_offset ? '' : height_rule,
    maxHeight: max_autoheight_offset
      ? `calc(100rvh - ${max_autoheight_offset})`
      : '',
  };
  React.useEffect(() => {
  ...
  return (
    <Div100vh
      id={id}
      className={className}
      style={is_disabled ? {} : height_style}
