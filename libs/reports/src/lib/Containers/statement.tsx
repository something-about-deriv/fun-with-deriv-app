...
type TGetStatementTableColumnsTemplate = ReturnType<
  typeof getStatementTableColumnsTemplate
>;
type TColIndex =
  | 'icon'
  | 'refid'
...
const DetailsComponent = ({
  ...
  const address_hash_match = /:\s([0-9a-zA-Z]+.{25,28})/gm.exec(
    message.split(/,\s/)[0]
  );
  const address_hash = address_hash_match?.[1];
  const blockchain_hash_match = /:\s([0-9a-zA-Z]+.{25,34})/gm.exec(
    message.split(/,\s/)[1]
  );
...
const getRowAction: TGetRowAction = (row_obj: TSource | TRow) => {
  ...
  if (row_obj.id && ['buy', 'sell'].includes(row_obj.action_type)) {
    ...
    action =
      ...
      !isForwardStarting(
        row_obj.shortcode,
        row_obj.purchase_time || row_obj.transaction_time
      )
    ...
    row_obj.desc &&
    ['deposit', 'transfer', 'adjustment', 'hold', 'release'].includes(
      row_obj.action_type
    )
  ) {
    action = {
  ...
  if (typeof action === 'object' && action?.message) {
    action.component = (
      <DetailsComponent
        ...
        action_type={row_obj.action_type}
      />
    );
  }
...
const Statement = observer(({ component_icon }: TStatement) => {
  ...
  const columns: TGetStatementTableColumnsTemplate =
    getStatementTableColumnsTemplate(currency);
  ...
  const mobileRowRenderer = ({
    ...
    <React.Fragment>
      ...
      <div className="data-list__row">
        <DataList.Cell
          row={row}
          column={columns_map.refid as TDataListCell['column']}
        />
        <DataList.Cell
      ...
      <div className="data-list__row">
        <DataList.Cell
          row={row}
          column={columns_map.date as TDataListCell['column']}
        />
        <DataList.Cell
          className="data-list__row-cell--amount"
          row={row}
      ...
      <div className="data-list__row">
        <DataList.Cell
          row={row}
          column={columns_map.balance as TDataListCell['column']}
        />
      </div>
    </React.Fragment>
  );
