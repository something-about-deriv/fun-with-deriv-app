...
type TGetProfitTableColumnsTemplate = ReturnType<
  typeof getProfitTableColumnsTemplate
>;
...
const getRowAction = (row_obj: { [key: string]: unknown }) => {
  ...
  return getSupportedContracts()[contract_type as TSupportedContractType] &&
    !isForwardStarting(
      row_obj.shortcode as string,
      Number(row_obj.purchase_time_unix)
    )
        ...
        component: (
          <Localize
            ...
            values={{
              trade_type_name:
                getUnsupportedContracts()[
                  contract_type as TUnsupportedContractType
                ]?.name,
            }}
          />
        ),
...
const ProfitTable = observer(({ component_icon }: TProfitTable) => {
  ...
  const columns: TGetProfitTableColumnsTemplate = getProfitTableColumnsTemplate(
    currency,
    data.length
  );

  const columns_map = Object.fromEntries(
    columns.map((column) => [column.col_index, column])
  ) as Record<
    TGetProfitTableColumnsTemplate[number]['col_index'],
    TGetProfitTableColumnsTemplate[number]
  >;

  const mobileRowRenderer: React.ComponentProps<
    typeof DataList
  >['rowRenderer'] = ({ row, is_footer }) => {
    const duration_type = /^(MULTUP|MULTDOWN)/.test(row?.shortcode)
      ? ''
      : row?.duration_type;
    const duration_classname = duration_type
      ? `duration-type__${duration_type.toLowerCase()}`
      : '';
    ...
    return (
      <>
          ...
          <div className={classNames('duration-type', duration_classname)}>
            <div
              className={classNames(
                'duration-type__background',
                `${duration_classname}__background`
              )}
            />
            <span className={`${duration_classname}__label`}>
        ...
        <div className="data-list__row">
          <DataList.Cell
            row={row}
            column={columns_map.transaction_id as TDataListCell['column']}
          />
          <DataList.Cell
            className="data-list__row-cell--amount"
            row={row}
        ...
        <div className="data-list__row">
          <DataList.Cell
            row={row}
            column={columns_map.purchase_time as TDataListCell['column']}
          />
          <DataList.Cell
            className="data-list__row-cell--amount"
            row={row}
        ...
        <div className="data-list__row">
          <DataList.Cell
            row={row}
            column={columns_map.sell_time as TDataListCell['column']}
          />
          <DataList.Cell
            className="data-list__row-cell--amount"
            row={row}
