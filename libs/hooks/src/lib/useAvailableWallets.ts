...
import { TSocketResponseData, useFetch } from '@deriv/api';
...
const useAvailableWallets = () => {
  ...
  const sortedWallets = React.useMemo(() => {
    ...
    // const { crypto, doughflow } =
    //   account_type_data?.get_account_types?.wallet || {};

    const wallet = account_type_data?.get_account_types?.wallet || {};
    let crypto: TSocketResponseData<'get_account_types'>['get_account_types']['wallet'][''] =
      { currencies: [] };
    let doughflow: TSocketResponseData<'get_account_types'>['get_account_types']['wallet'][''] =
      { currencies: [] };
    ...
    if (wallet['doughflow']) {
      doughflow = wallet['doughflow'];
    }
    const crypto_currencies: string[] = crypto?.currencies || [];
    const fiat_currencies: string[] = doughflow?.currencies || [];

    if (!crypto_currencies || !fiat_currencies) return null;
    const available_currencies = [...fiat_currencies, ...crypto_currencies];
    const non_virtual_wallets = added_wallets?.filter(
      (wallet) => !wallet.is_virtual
    );
    ...
    const available_wallets = available_currencies
      .filter(
        (currency) =>
          !modified_wallets?.some((wallet) => wallet.currency === currency)
      )
      .map((currency) => ({
        currency,
