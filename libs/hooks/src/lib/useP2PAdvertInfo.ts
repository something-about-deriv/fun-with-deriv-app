...
import { TSocketRequestQueryOptions } from '@deriv/api';

/**
 * This custom hook returns the advert info for a specific advert by calling 'p2p_advert_info' endpoint
 */
const useP2PAdvertInfo = (
  id: string,
  options: TSocketRequestQueryOptions<'p2p_advert_info'>
) => {
  const { data, ...rest } = useFetch('p2p_advert_info', {
    payload: { id },
    options,
  });
