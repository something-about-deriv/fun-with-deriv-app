import {
  ...
  TOptions,
  getPreBuildDVRs,
} from './declarative-validation-rules';
...
export const getDefaultFields = (
  ...
  schema: TSchema | Record<string, never>
) => {
  const output: { [key: string]: string | number | boolean } = {};
  Object.entries(filterByLandingCompany(landing_company, schema)).forEach(
    ([field_name, opts]) => {
...
export const filterByLandingCompany = (
  landing_company: string,
  schema: TSchema | Record<string, never>
): TSchema =>
  fromEntries(
    Object.entries(schema).filter(([, opts]) =>
      opts.supported_in.includes(landing_company)
    )
  );
...
export const generateValidationFunction = (
  ...
  const rules_schema = landing_company
    ? filterByLandingCompany(landing_company, schema)
    : schema;
