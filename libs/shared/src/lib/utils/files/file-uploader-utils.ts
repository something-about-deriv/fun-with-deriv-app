...
import {
  ...
  getFormatFromMIME,
  TImage,
} from './image/image_utility';

export type TSettings = Parameters<
  ReturnType<typeof useMutation<'document_upload'>>['mutate']
>[0]['payload'];
...
export const readFiles = (
  ...
  files.forEach((f) => {
    ...
    const promise = new Promise<Partial<TFileObject> | { message: string }>(
      (resolve) => {
        fr.onload = () => {
          const file_metadata = {
            filename: f.name,
            ...
            file_size: f.size,
            documentType:
              settings?.document_type ?? DOCUMENT_TYPES.utility_bill,
