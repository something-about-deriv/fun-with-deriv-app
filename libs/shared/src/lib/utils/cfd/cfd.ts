...
} from '../constants/jurisdictions-config';
...
export const getAccountTypeFields = ({
  category,
  type,
}: TGetAccountTypeFields) => {
...
export const getCFDAccountDisplay = ({
  ...
  const cfd_account_key = getCFDAccountKey({
    market_type,
    sub_account_type,
    platform,
    shortcode,
  ...
  if (market_type === 'synthetic' && platform === CFD_PLATFORMS.DXTRADE)
...
export const getCFDAccount = ({
  ...
  let cfd_account_key = getCFDAccountKey({
    market_type,
    sub_account_type,
    platform,
  });
...
export const getAccountListKey = (
  account: TAccount,
  platform: TPlatform,
  shortcode?: TShortcode
) => {
...
export const isLandingCompanyEnabled = ({
  landing_companies,
  platform,
  type,
}: TIsLandingCompanyEnabled) => {
  if (platform === CFD_PLATFORMS.MT5) {
    if (type === 'gaming')
      return !!landing_companies?.mt_gaming_company?.financial;
    if (type === 'financial')
      return !!landing_companies?.mt_financial_company?.financial;
    if (type === 'financial_stp')
      return !!landing_companies?.mt_financial_company?.financial_stp;
  } else if (platform === CFD_PLATFORMS.DXTRADE) {
    if (type === 'all')
      return !!landing_companies?.dxtrade_all_company?.standard;
    if (type === 'gaming')
      return !!landing_companies?.dxtrade_gaming_company?.standard;
    if (type === 'financial')
      return !!landing_companies?.dxtrade_financial_company?.standard;
  }
  return false;
};
...
export const getAuthenticationStatusInfo = (
  account_status: GetAccountStatus
): TAuthenticationStatusInfo => {
  const risk_classification = account_status?.risk_classification;

  const poa_status: string =
    account_status?.authentication?.document?.status || '';
  const poi_status: string =
  ...
  const is_authenticated_with_idv_photoid = account_status?.status?.includes(
  ...
  const poi_or_poa_not_submitted: boolean =
    poa_not_submitted || poi_not_submitted;
  const poi_and_poa_not_submitted: boolean =
    poa_not_submitted && poi_not_submitted;

  //maltainvest

  // mf = maltainvest: only require onfido and manual
  const mf_jurisdiction_statuses: string[] = [
    onfido_status,
    manual_status,
  ].filter((status: string | undefined) => status) as string[];
  // bvi_labuan_vanuatu jurisdictions: require idv, onfido and manual
  const bvi_labuan_vanuatu_jurisdiction_statuses: string[] = [
    idv_status,
    onfido_status,
    manual_status,
  ].filter((status) => status) as string[];

  const poi_verified_for_maltainvest: boolean =
    mf_jurisdiction_statuses.includes('verified');
  const poi_acknowledged_for_maltainvest: boolean =
    mf_jurisdiction_statuses.some((status) =>
      acknowledged_status.includes(status)
    );
  const poi_pending_for_maltainvest: boolean =
    mf_jurisdiction_statuses.some((status) => status === 'pending') &&
    !poi_verified_for_maltainvest;

  const need_poi_for_maltainvest = !poi_acknowledged_for_maltainvest;
  const poi_not_submitted_for_maltainvest: boolean =
    mf_jurisdiction_statuses.every((status) => status === 'none');
  ...
  const poi_poa_verified_for_maltainvest =
    poi_verified_for_maltainvest && poa_verified;

  //bvi-labuan-vanuatu
  let poi_acknowledged_for_bvi_labuan_vanuatu: boolean =
    bvi_labuan_vanuatu_jurisdiction_statuses.some((status) =>
      acknowledged_status.includes(status)
    );
  ...
  const need_poi_for_bvi_labuan_vanuatu =
    !poi_acknowledged_for_bvi_labuan_vanuatu;
  const poi_not_submitted_for_bvi_labuan_vanuatu: boolean =
    bvi_labuan_vanuatu_jurisdiction_statuses.every(
      (status) => status === 'none'
    );

  const poi_verified_for_bvi_labuan_vanuatu: boolean =
    bvi_labuan_vanuatu_jurisdiction_statuses.includes('verified');

  const poi_pending_for_bvi_labuan_vanuatu: boolean =
    bvi_labuan_vanuatu_jurisdiction_statuses.includes('pending') &&
  ...
  const poi_poa_verified_for_bvi_labuan_vanuatu: boolean =
    poi_verified_for_bvi_labuan_vanuatu && poa_verified;
...
export const getFormattedJurisdictionCode = (
  jurisdiction_code?: (typeof Jurisdiction)[keyof typeof Jurisdiction]
) => {
  let formatted_label = '';
...
export const getFormattedJurisdictionMarketTypes = (
  jurisdiction_market_type:
    | (typeof JURISDICTION_MARKET_TYPES)[keyof typeof JURISDICTION_MARKET_TYPES]
    | TMarketType
) => {
...
export const getMT5AccountTitle = ({
  account_type,
  jurisdiction,
}: TGetMT5AccountTitle) => {
  return `${getCFDPlatformNames(
    CFD_PLATFORMS.MT5
