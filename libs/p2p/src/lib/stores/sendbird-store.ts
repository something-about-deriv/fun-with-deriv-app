...
import { TCoreStores } from '@deriv/stores';
...
export default class SendbirdStore extends BaseStore {
  ...
  sendbird_api: ReturnType<
    typeof SendbirdChat.init<GroupChannelModule[]>
  > | null = null;
  ...
  get has_chat_info() {
    return (
      this.chat_info.app_id && this.chat_info.user_id && this.chat_info.token
    );
  }
  ...
  replaceChannelMessage(
    idx_to_replace: number,
    num_items_to_delete: number,
    chat_message: ChatMessage
  ) {
    this.chat_messages.splice(
      idx_to_replace,
      num_items_to_delete,
      chat_message
    );
  ...
  async initialiseChatWsConnection() {
    try {
      ...
      const send_bird_user = await this.sendbird_api.connect(
        this.chat_info.user_id,
        this.chat_info.token
      );
      if (!send_bird_user) {
        this.setChatError();
      } else {
  ...
  async initialiseOrderChannel() {
    ...
    this.setIsChatLoading(true);
    try {
      const group_channel = await this.sendbird_api?.groupChannel.getChannel(
    ...
    try {
      ...
      if (chat_messages && chat_messages.length > 0) {
        this.setChannelMessages(
          chat_messages.map((msg) => convertFromChannelMessage(msg))
        );
        this.messages_ref?.current?.scrollTo(
          0,
          this.messages_ref.current.scrollHeight
        );
      }
    } catch (error) {
      // eslint-disable-next-line no-console
  ...
  async getPreviousMessages(timestamp: number | null = null) {
    ...
    const messages_timestamp =
      timestamp ??
      toMoment(this.root_store.general_store.server_time.get()).utc().valueOf();

    const retrieved_messages =
      await this.active_chat_channel?.getMessagesByTimestamp(
        messages_timestamp,
        {
  ...
  handleP2pAdvertiserInfo(
    response: Record<string, P2PAdvertiserCreate | P2PAdvertiserInfo>
  ) {
    ...
    const p2p_advertiser_info =
      response.p2p_advertiser_create || response.p2p_advertiser_info;

    const getSendbirdServiceToken = () => {
      requestWS({ service: 'sendbird', service_token: 1 }).then(
        (service_token_response) => {
          ...
          const expiry_moment = epochToMoment(
            service_token.sendbird.expiry_time
          );
          const delay_ms = expiry_moment.diff(
            toMoment(server_time.get()).clone().subtract(1, 'hour')
          );

          this.service_token_timeout = setTimeout(
      ...
      if (this.messages_ref.current.scrollTop === 0) {
        const oldest_message_timestamp = this.chat_messages.reduce(
          (prev_created_at, chat_message) =>
            chat_message.created_at < prev_created_at
  ...
  registerEventListeners() {
    ...
    return () =>
      window.removeEventListener('focus', markMessagesAsReadCheckScroll);
  }
  ...
  sendMessage(message: string, custom_type = '') {
    ...
    const msg_identifier = `${Date.now()}${message.substring(0, 9)}${
      this.chat_messages.length
    }`;

    // Add a placeholder message with a pending indicator
    const placeholder_msg_options = {
      created_at: toMoment(this.root_store.general_store.server_time.get())
        .utc()
        .valueOf(),
    ...
    this.active_chat_channel
      ...
      .onSucceeded((channel_message) => {
        const msg_idx = this.chat_messages.findIndex(
          (msg) => msg.id === msg_identifier
        );
        if (channel_message.isUserMessage()) {
          this.replaceChannelMessage(
            msg_idx,
            1,
            convertFromChannelMessage(channel_message)
          );
          this.messages_ref?.current?.scrollTo(
            0,
            this.messages_ref.current.scrollHeight
  ...
  terminateChatWsConnection() {
    if (
      this.sendbird_api &&
      ((this.file_upload_properties && this.is_upload_complete) ||
