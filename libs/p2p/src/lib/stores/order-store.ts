...
export default class OrderStore {
  constructor(root_store) {
    ...
    reaction(
      ...
      (orders) => {
        ...
        this.root_store.general_store.handleNotifications(
  ...
  getWebsiteStatus(should_show_cancel_modal) {
    requestWS({ website_status: 1 }).then((response) => {
      if (response.error) {
        ...
        this.setCancellationBlockDuration(
          p2p_config.cancellation_block_duration
        );
      ...
      requestWS({
        ...
        if (!response?.error) {
          ...
          if (response?.echo_req?.active === active) {
            ...
            this.setHasMoreItemsToLoad(
              list.length >= general_store.list_item_limit
            );
          ...
          response?.error?.code === api_error_codes.PERMISSION_DENIED
        ) {
          // @ts-expect-error fix this
  ...
  async onOrdersUpdate() {
    if (this.order_id) {
      ...
      await requestWS({ p2p_order_info: 1, id: this.order_id }).then(
        (response) => {
          if (!response?.error) {
  ...
  setQueryDetails(input_order) {
    ...
    if (order_information?.payment_method_details) {
      this.setOrderPaymentMethodDetails(
        Object.values(order_information?.payment_method_details)
      );
    }
    ...
    const { notifications } =
      // @ts-expect-error fix this
      this.root_store.general_store.getLocalStorageSettingsForLoginId();

    if (notifications.length) {
      const notification = notifications.find(
        (n) => n.order_id === order_information.id
      );
  ...
  syncOrder(p2p_order_info) {
    ...
    const order_idx = this.orders.findIndex(
      (order) => order.id === p2p_order_info.id
    );
    ...
    if (this.order_id === null) {
      ...
      if (order_idx === -1) {
        ...
        (get_order_status.is_completed_order &&
          get_order_status.has_review_details) ||
        !get_order_status.is_reviewable
      ) {
        Object.assign(this.orders[order_idx], p2p_order_info);
      } else if (
        get_order_status.is_disputed_order ||
        get_order_status.is_active_order
      ) {
        Object.assign(this.orders[order_idx], p2p_order_info);
    ...
    if (
      ...
      general_store.external_stores?.notifications.removeNotificationMessage({
        ...
        if (response) {
          if (!response.error) {
            ...
            if (
              ...
              if (
                general_store.isCurrentModal('InvalidVerificationLinkModal')
              ) {
                general_store.hideModal();
              }
              this.setVerificationLinkErrorMessage(message);
