...
const App = () => {
  ...
  React.useEffect(() => {
    ...
    general_store.setExternalStores({
      client,
      common,
      modules,
      notifications,
    ...
    if (is_mobile) {
      ...
      if (url_params.has('code')) {
        ...
        setCodeParam(
          ...
          localStorage.getItem('verification_code.p2p_order_confirm')
        );
      }
    }
  ...
  const setQueryOrder = React.useCallback(
    (input_order_id) => {
      ...
      if (
        current_query_params.has('order_id') ||
        current_query_params.has('order')
      ) {
  ...
  React.useEffect(() => {
    ...
    setP2POrderProps({
      ...
      setIsRatingModalOpen: (is_open) =>
        is_open
          ? general_store.showModal({ key: 'RatingModal' })
