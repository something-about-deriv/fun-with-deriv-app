...
// import { useTraderStore } from 'src/lib/trader-providers';
...
const getTradingTimes = async (
  target_time: TradingTimesRequest['trading_times']
) => {
...
const getSymbol = (
  ...
  trading_times: NonNullable<
    DeepRequired<TradingTimesResponse['trading_times']>
  >
) => {
  let symbol;
...
const MarketCountdownTimer = observer(
  ({
    ...
    onMarketOpen,
    symbol,
  }: TMarketCountDownTimer) => {
    const { active_symbols } = useTraderStore();
    const isMounted = useIsMounted();
    const [when_market_opens, setWhenMarketOpens] =
      React.useState<TWhenMarketOpens>({} as TWhenMarketOpens);
    const [time_left, setTimeLeft] = React.useState(
      calculateTimeLeft(when_market_opens?.remaining_time_to_open)
    );
    ...
    React.useEffect(() => {
      if (
        !is_main_page ||
        (is_main_page && isMarketClosed(active_symbols, symbol))
      ) {
        setLoading(true);
        const whenMarketOpens = async (
          ...
          const api_response = await getTradingTimes(
            target_date.format('YYYY-MM-DD')
          );
          if (!api_response.api_initial_load_error) {
            const returned_symbol = getSymbol(
              target_symbol,
              api_response.trading_times
            );
            ...
            const getUTCDate = (hour: string) =>
              new Date(`${date_str}${hour}Z`);
    ...
    React.useEffect(() => {
      ...
      if (when_market_opens?.remaining_time_to_open) {
        timer = setTimeout(() => {
          ...
          if (
            +new Date(when_market_opens.remaining_time_to_open) - +new Date() <
            1000
          ) {
            setLoading(true);
    ...
    if (Object.keys(time_left).length) {
      const hours = (Number(time_left.days) * 24 + Number(time_left.hours))
        .toString()
        .padStart(2, '0');
