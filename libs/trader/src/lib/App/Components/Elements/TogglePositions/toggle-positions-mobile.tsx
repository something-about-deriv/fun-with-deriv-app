...
import PositionsModalCard from '../../../Components/Elements/PositionsDrawer/positions-modal-card';
...
const TogglePositionsMobile = observer(
  ({
    ...
    const displayed_positions = filtered_positions
      .filter((p) =>
        hidden_positions_ids.every(
          (hidden_position_id) =>
            hidden_position_id !== p.contract_info.contract_id
        )
      )
      .slice(0, 5);
    ...
    const closeModal = () => {
      setHiddenPositionsIds([
        ...new Set([...hidden_positions_ids, ...closed_positions_ids]),
      ]);
      togglePositionsDrawer();
    };
