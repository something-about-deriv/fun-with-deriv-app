...
describe('TogglePositionsMobile component', () => {
  ...
  it('should display an empty Modal when is_positions_drawer_on === true and is_empty === true', () => {
    ...
    render(
      mockTogglePositionsMobile(mock_root_store, {
    ...
    expect(
      screen.getByText(/You have no open positions for this asset./i)
    ).toBeInTheDocument();
  });
  ...
  it('should display no more than 5 recent positions', () => {
    const positions_pair: React.ComponentProps<
      typeof TogglePositionsMobile
    >['filtered_positions'] = [
      {
        contract_info: mockContractInfo({
          contract_id: 1,
