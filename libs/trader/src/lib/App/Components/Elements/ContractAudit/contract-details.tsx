...
const ContractDetails = ({
  ...
  const show_duration =
    !isAccumulatorContract(contract_type) || !isNaN(Number(contract_end_time));
  const show_payout_per_point = isTurbosContract(contract_type) || is_vanilla;
  const ticks_label =
    Number(tick_count) < 2 ? localize('tick') : localize('ticks');
  ...
  const getLabel = () => {
    if (
      isUserSold(contract_info) &&
      isEndedBeforeCancellationExpired(contract_info)
    )
      return localize('Deal cancellation');
    if (isUserCancelled(contract_info))
      return localize('Deal cancellation (executed)');
    if (isCancellationExpired(contract_info))
      return localize('Deal cancellation (expired)');
    ...
    <ThemedScrollbars is_bypassed={isMobile()}>
      <div className="contract-audit__tabs-content">
        <ContractAuditItem
          ...
          value2={
            sell
              ? localize('{{sell_value}} (Sell)', { sell_value: sell })
              : undefined
          }
        />
          ...
          <React.Fragment>
            ...
            {show_payout_per_point && (
              <ContractAuditItem
                ...
                value={
                  display_number_of_contracts
                    ? `${display_number_of_contracts} ${getCurrencyDisplayCode(
                        currency
                      )}`
                    : ' - '
                }
        ...
        {!isDigitType(contract_type) && (
          <ContractAuditItem
            ...
            value2={
              toGMTFormat(epochToMoment(Number(entry_tick_time))) || ' - '
            }
          />
        )}
        ...
        {!isNaN(Number(contract_end_time)) && (
          <ContractAuditItem
            ...
            icon={
              <Icon
                icon="IcContractExitTime"
                color={is_profit ? 'green' : 'red'}
                size={24}
              />
            }
            label={localize('Exit time')}
            value={
              toGMTFormat(epochToMoment(Number(contract_end_time))) || ' - '
            }
          />
        )}
      </div>
