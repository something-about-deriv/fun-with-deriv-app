...
type TContractAudit = Partial<
  Pick<
    ...
    'is_accumulator' | 'is_turbos' | 'is_multiplier' | 'is_vanilla'
  >
> & {
...
const ContractAudit = ({
  ...
  const [update_history, setUpdateHistory] =
    React.useState<TContractUpdateHistory>([]);
  ...
  const onTabItemClick = (tab_index: number) => {
    ...
    if (tab_index) {
      WS.contractUpdateHistory(contract_id).then((response: TResponse) => {
        setUpdateHistory(
          getSortedUpdateHistory(response.contract_update_history)
        );
      });
    }
  };
  ...
  return (
    <div className="contract-audit__wrapper">
      <Tabs
        top
        className="contract-audit__tabs"
