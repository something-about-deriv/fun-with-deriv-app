...
const RouteWithSubRoutes = (route: TRouteWithSubRoutesProps) => {
  const validateRoute = (pathname: string) => {
    ...
    if (route.path?.includes(':')) {
      const static_pathname = pathname.substring(
        0,
        pathname.lastIndexOf('/') + 1
      );
      return (
        static_pathname === route.path.substring(0, route.path.indexOf(':'))
      );
    }
    return (
      route.path === pathname ||
      !!route.routes?.find((r) => pathname === r.path)
    );
  };
  ...
  const renderFactory = (props: RouteComponentProps) => {
    ...
    if (route.component === Redirect) {
      ...
      const default_subroute = route.routes
