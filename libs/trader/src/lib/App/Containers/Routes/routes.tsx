...
import BinaryRoutes from '../../Components/Routes/binary-routes';
...
const checkRoutingMatch = (
  route_list: Array<string | undefined>,
  path = ''
) => {
  return route_list.some(
    (route) => !!matchPath(path, { path: route, exact: true })
  );
};
...
const tradePageMountingMiddleware = ({
  ...
  if (action === 'PUSH' || action === 'POP') {
    ...
    const has_match = match_patterns.some(
      (pattern) =>
        checkRoutingMatch(pattern.from, path_from) &&
        checkRoutingMatch(pattern.to, path_to)
    );
...
//   loader: () =>
//     import(
//       /* webpackChunkName: "error-component" */ 'App/Components/Elements/Errors'
//     ),
//   loading: () => <UILoader />,
//   render(loaded, props) {
...
const Routes = observer(({ history, passthrough }: TRoutesProps) => {
  ...
  React.useEffect(() => {
    if (setPromptHandler) {
      setPromptHandler(
        true,
        (route_to: RouteComponentProps['location'], action: string) => {
            ...
            match_patterns: [
              {
                from: getRoutesConfig()
                  ...
                  .filter(
                    (path) =>
                      path && path !== routes.mt5 && path !== routes.dxtrade
                  ),
