...
type TValidationRules =
  | ReturnType<typeof getValidationRules>
  | Record<string, never>;
...
export default class BaseStore {
  ...
  constructor(options = {} as TBaseStoreOptions) {
    ...
    const has_local_or_session_storage =
      local_storage_properties?.length || session_storage_properties?.length;
  ...
  getSnapshot(properties: string[]): object {
    ...
    if (properties?.length) {
      snapshot = properties.reduce(
        (result, p) =>
          Object.assign(result, { [p]: snapshot[p as keyof this] }),
        {} as this
      );
    }
  ...
  setupReactionForLocalStorage() {
    if (this.local_storage_properties.length) {
      reaction(
        ...
        () =>
          this.saveToStorage(
            this.local_storage_properties,
            BaseStore.STORAGES.LOCAL_STORAGE
          )
      );
    }
  }
   ...
   */
  retrieveFromStorage() {
    const local_storage_snapshot = JSON.parse(
      String(localStorage.getItem(this.store_name))
    );
    const session_storage_snapshot = JSON.parse(
      String(sessionStorage.getItem(this.store_name))
    );
  ...
  setValidationErrorMessages(propertyName: string, messages: string[]) {
    const is_different = () =>
      !!this.validation_errors[propertyName]
        ...
        .concat(
          messages.filter(
            (x) => !this.validation_errors[propertyName].includes(x)
          )
        ).length;
  ...
  setValidationRules(rules: TValidationRules = {}): void {
    Object.keys(rules).forEach((key) => {
      this.addRule<keyof TValidationRules>(
        key as keyof TValidationRules,
        rules[key as keyof TValidationRules]
      );
    });
  }
   ...
   *
   */
  addRule<T extends keyof TValidationRules>(
    property: T,
    rules: TValidationRules[T]
  ) {
  ...
  validateProperty<T extends BaseStore>(property: string, value: T[keyof T]) {
    ...
    const trigger =
      'trigger' in validation_rules_for_property
        ? validation_rules_for_property.trigger
        : undefined;
    const inputs = { [property]: value ?? this[property as keyof this] };
    const validation_rules = {
      [property]:
        'rules' in validation_rules_for_property
          ? validation_rules_for_property.rules
          : [],
    };

    if (!!trigger && Object.hasOwnProperty.call(this, trigger)) {
      const validation_rules_for_trigger =
        this.validation_rules[trigger as keyof TValidationRules];
      inputs[trigger] = this[trigger as keyof this];
      validation_rules[trigger] =
        'rules' in validation_rules_for_trigger
          ? validation_rules_for_trigger.rules
          : [];
    }
        ...
        () => {
          try {
            ...
            if (result?.then && typeof result.then === 'function') {
              ...
              throw new Error(
                'Switching account listeners are required to return a promise.'
              );
            }
          } catch (error) {
        ...
        () => {
          try {
            ...
            if (result?.then && typeof result.then === 'function') {
              ...
              throw new Error(
                'Pre-switch account listeners are required to return a promise.'
              );
            }
          } catch (error) {
            // there is no listener currently active. so we can just ignore the error raised from treating
      ...
      async () => {
        try {
          ...
          if (result?.then && typeof result.then === 'function') {
            ...
            throw new Error(
              'Logout listeners are required to return a promise.'
            );
          }
        } catch (error) {
          // there is no listener currently active. so we can just ignore the error raised from treating
      ...
      async () => {
        try {
          ...
          if (result?.then && typeof result.then === 'function') {
            ...
            throw new Error(
              'Client init listeners are required to return a promise.'
            );
          }
        } catch (error) {
          // there is no listener currently active. so we can just ignore the error raised from treating
          ...
          if (result?.then && typeof result.then === 'function') {
            result.then(() => {
              ...
              this.onRealAccountSignupEnd(
                this.real_account_signup_ended_listener
              );
            });
          } else {
            throw new Error(
              'Real account signup listeners are required to return a promise.'
            );
          }
        } catch (error) {
          // there is no listener currently active. so we can just ignore the error raised from treating
