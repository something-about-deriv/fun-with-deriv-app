...
export const onChangeStartDate = async (store: TTradeStore) => {
  ...
  start_time =
    start_time || server_time?.clone().add(6, 'minute').format('HH:mm'); // when there is not a default value for start_time, it should be set more than 5 min after server_time
  ...
  const obj_start_time = ContractType.getStartTime(
    sessions,
    start_date,
    start_time
  );

  const obj_duration_units_list = ContractType.getDurationUnitsList(
    contract_type,
    contract_start_type
  );
  const duration_units_list = obj_duration_units_list.duration_units_list;
  const obj_duration_unit = ContractType.getDurationUnit(
    duration_unit,
    contract_type,
    contract_start_type
  );

  const obj_expiry_type = ContractType.getExpiryType(
    duration_units_list,
    expiry_type
  );
  expiry_type = obj_expiry_type.expiry_type;
  const obj_expiry_date = ContractType.getExpiryDate(
    duration_units_list,
    store.expiry_date,
    expiry_type,
    start_date
  ...
  const obj_duration_min_max = ContractType.getDurationMinMax(
    contract_type,
    contract_start_type
  );
