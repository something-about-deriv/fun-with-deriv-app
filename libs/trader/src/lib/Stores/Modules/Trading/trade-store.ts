...
} from '../../../Stores/Modules/Trading/Constants/validation-rules';
import { ContractType } from '../../../Stores/Modules/Trading/Helpers/contract-type';
import {
  isDigitContractType,
  isDigitTradeType,
} from '../../../Modules/Trading/Helpers/digits';
...
type TContractDataForGTM = Omit<
  Partial<PriceProposalRequest>,
  'cancellation' | 'limit_order'
> &
...
type TBarriersData =
  | Record<string, never>
  | { barrier: string; barrier_choices: string[] };

const store_name = 'trade_store';
const g_subscribers_map: Partial<
  Record<string, ReturnType<typeof WS.subscribeTicksHistory>>
> = {}; // blame amin.m
...
export default class TradeStore extends BaseStore {
  ...
  constructor({ root_store }: { root_store: TCoreStores }) {
    ...
    reaction(
      () => this.symbol,
      () => {
        const date = resetEndTimeOnVolatilityIndices(
          this.symbol,
          this.expiry_type
        );
        if (date) {
    ...
    reaction(
      ...
      () => {
        this.contract_expiry_type =
          this.duration_unit === 't' ? 'tick' : 'intraday';
      }
    );
    reaction(
    ...
    return (
      ...
      !!this.root_store.portfolio.active_positions.find(
        ({ contract_info, type }) =>
          ...
          contract_info.underlying === this.symbol
      )
    );
  }
  ...
  resetAccumulatorData() {
    if (
      !isEmptyObject(this.root_store.contract_trade.accumulator_barriers_data)
    ) {
  ...
  async loadActiveSymbols(
    should_set_default_symbol = true,
  ...
  async setActiveSymbols() {
    const is_on_mf_account =
      this.root_store.client.landing_company_shortcode === 'maltainvest';
    ...
    if (!active_symbols?.length) {
      ...
      if (is_on_mf_account) {
        showDigitalOptionsUnavailableError(showError, {
          ...
          title: localize(
            '{{platform_name_trader}} is not available for this account',
            {
  ...
  async prepareTradeStore(should_set_default_symbol = true) {
    this.initial_barriers = {
      barrier_1: this.barrier_1,
      barrier_2: this.barrier_2,
    };
      ...
      await this.processNewValuesAsync(
        {
          ...
          currency:
            ...
            this.root_store.client.default_currency,
        },
        true,
        null,
        false
  ...
  async resetPreviousSymbol() {
    this.setMarketStatus(
      isMarketClosed(this.active_symbols, this.previous_symbol)
    );
  ...
  get barriers_flattened() {
    return (
      this.root_store.portfolio.barriers &&
      toJS(this.root_store.portfolio.barriers)
    );
  }
  ...
  setMainBarrier = (proposal_info: PriceProposalRequest) => {
    ...
    if (isBarrierSupported(contract_type)) {
      ...
      this.main_barrier = new ChartBarrierStore(
        ...
        {
          color: this.hovered_barrier
            ? getHoveredColor(contract_type)
            : BARRIER_COLORS.BLUE,
        ...
        action((response: TResponse<Buy, BuyContractResponse, 'buy'>) => {
          ...
          if (response.error) {
            ...
            if (response.error.code !== 'InvalidToken') {
              ...
              if (
                isMobile() &&
                this.root_store.common?.services_error?.type === 'buy'
              ) {
                setTimeout(() => {
            ...
            if (contract_id) {
              ...
              const { category, underlying } =
                extractInfoFromShortcode(shortcode);
              const is_digit_contract = isDigitContractType(
  ...
  updateStore(new_state: Partial<TradeStore>) {
    Object.keys(cloneObject(new_state) || {}).forEach((key) => {
      if (
        ...
        return;
      if (
        JSON.stringify(this[key as keyof this]) ===
        JSON.stringify(new_state[key as keyof TradeStore])
      ) {
        ...
        this[key as 'currency'] = new_state[
          key as keyof TradeStore
        ] as TradeStore['currency'];
  ...
  async processNewValuesAsync(
    ...
    if (
      ...
      obj_new_values.contract_type === 'rise_fall' &&
      !!this.is_equal
    ) {
    ...
    if (
      is_changed_by_user &&
      /\bcurrency\b/.test(Object.keys(obj_new_values) as unknown as string)
    ) {
      ...
      const should_reset_stake =
        ...
        isCryptocurrency(obj_new_values.currency ?? '') !==
          isCryptocurrency(prev_currency);
    ...
    if (Object.keys(obj_new_values).includes('symbol')) {
      ...
      this.setMarketStatus(
        isMarketClosed(this.active_symbols, obj_new_values.symbol ?? '')
      );
    ...
    this.root_store.ui.setHasOnlyForwardingContracts(
      has_only_forward_starting_contracts
    );
    ...
    if (
      is_changed_by_user ||
      /\b(symbol|contract_types_list)\b/.test(
        Object.keys(new_state) as unknown as string
      )
    ) {
      this.updateStore({
      ...
      if (
        obj_new_values.contract_type &&
        this.root_store.ui.is_advanced_duration
      ) {
      ...
      if (
        /\b(contract_type|currency)\b/.test(
          Object.keys(new_state) as unknown as string
        )
      ) {
        this.validateAllProperties();
  ...
  get is_synthetics_trading_market_available() {
    return !!this.active_symbols?.find(
      (item) =>
        ...
        !isMarketClosed(this.active_symbols, item.symbol)
    );
  }
  ...
  pushPurchaseDataToGtm(contract_data: TContractDataForGTM) {
    const data = {
      ...
      settings: {
        ...
        positions_drawer: this.root_store.ui.is_positions_drawer_on
          ? 'open'
          : 'closed',
        chart: {
          toolbar_position: this.root_store.ui.is_chart_layout_default
            ? 'bottom'
            : 'left',
          chart_asset_info: this.root_store.ui.is_chart_asset_info_visible
            ? 'visible'
            : 'hidden',
  ...
  onProposalResponse(
    response: TResponse<PriceProposalRequest, PriceProposalResponse, 'proposal'>
  ) {
    const { contract_type } = response.echo_req;
    const prev_proposal_info =
      getPropertyValue(this.proposal_info, contract_type) || {};
    const obj_prev_contract_basis =
      getPropertyValue(prev_proposal_info, 'obj_contract_basis') || {};
    ...
    if (this.is_multiplier && this.proposal_info && this.proposal_info.MULTUP) {
      const { commission, cancellation, limit_order } =
        this.proposal_info.MULTUP;
    ...
    if (this.is_accumulator && this.proposal_info?.ACCU) {
      ...
      const { updateAccumulatorBarriersData } =
        this.root_store.contract_trade || {};
    ...
    if (response.error) {
      ...
      if (
        (this.is_turbos || this.is_vanilla) &&
        response.error.details?.barrier_choices
      ) {
        const { barrier_choices, max_stake, min_stake } =
        ...
        if (!this.barrier_choices.includes(this.barrier_1)) {
          ...
          this.barrier_1 = this.is_vanilla
            ? this.barrier_choices[index]
            : this.barrier_choices[0];
          this.onChange({
            target: {
              name: 'barrier_1',
      ...
      if (this.is_turbos || this.is_vanilla) {
        const { max_stake, min_stake, barrier_choices } =
          response.proposal ?? {};
        this.setBarrierChoices(barrier_choices as string[]);
        this.setStakeBoundary(contract_type, min_stake, max_stake);
      }
  ...
  onAllowEqualsChange() {
    this.processNewValuesAsync(
      { contract_type: this.is_equal ? 'rise_fall_equal' : 'rise_fall' },
      true
    );
  }
  ...
  changeDurationValidationRules() {
    ...
    const index = this.validation_rules.duration.rules?.findIndex(
      (item) => item[0] === 'number'
    );
    ...
    if (limits) {
      ...
      if (Number(index) > -1 && this.validation_rules.duration.rules) {
        this.validation_rules.duration.rules[Number(index)][1] =
          duration_options;
      } else {
        this.validation_rules.duration.rules?.push([
          'number',
          duration_options,
        ]);
      }
  ...
  clientInitListener() {
    this.initAccountCurrency(
      this.root_store.client.currency || this.root_store.client.default_currency
    );
    return Promise.resolve();
  }
  ...
  wsSubscribe = (
    ...
    callback: (response: TTicksHistoryResponse) => void
  ) => {
  ...
  wsSendRequest = (
    req: TradingTimesRequest | ActiveSymbolsRequest | ServerTimeRequest
  ) => {
    if ('time' in req) {
  ...
  chartStateChange(state: string, option?: TChartStateChangeOption) {
    ...
    if (state === STATE_TYPES.SET_CHART_MODE) {
      if (!isNaN(Number(option?.granularity))) {
        this.root_store.contract_trade.updateGranularity(
          Number(option?.granularity)
        );
      }
    ...
    const { data, event_type } = getChartAnalyticsData(
      state as keyof typeof STATE_TYPES,
      option
    ) as TPayload;
    if (data) {
  ...
  async getFirstOpenMarket(markets_to_search: string[]) {
    ...
    if (error) {
      this.root_store.common.showError({
        message: localize('Trading is unavailable at this time.'),
      });
      return undefined;
    }
