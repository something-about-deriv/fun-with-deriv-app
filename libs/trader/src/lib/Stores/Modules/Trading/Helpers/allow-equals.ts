...
export const hasDurationForCallPutEqual = (
  ...
  if (!contract_type_list || !duration_unit || !contract_start_type)
    return false;

  const contract_list = Object.keys(contract_type_list || {}).reduce<string[]>(
    (key, list) => {
      const item: THasDurationForCallPutEqual['contract_type_list']['Ups & Downs'] =
  ...
  if (hasCallPutEqual(contract_type_list)) {
    const found = contract_duration_list.filter(
      (contract) => contract?.rise_fall_equal
    );
    if (found.length > 0) {
      return found[0].rise_fall_equal.some(
        (duration: { value: string }) => duration.value === duration_unit
      );
    }
  }
...
export const isRiseFallEqual = (contract_type: string) =>
  /^(rise_fall|rise_fall_equal)$/.test(contract_type);
