...
const processInSequence = async (
  store: TTradeStore,
  functions:
    | ReturnType<typeof getMethodsList>
    | ReturnType<typeof getExpiryMethodsList>
) => {
...
const getMethodsList = (
  ...
  const filtered_keys = Object.keys(new_state).filter((key) =>
    /\b(symbol|contract_type|is_equal)\b/.test(key)
  );
  return [
