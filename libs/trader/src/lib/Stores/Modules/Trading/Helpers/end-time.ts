...
type TTime = {
  server_time: NonNullable<
    ReturnType<typeof useStore>['common']['server_time']
  >;
...
const getClosestTime = (
  ...
  return moment_time.minute(
    Math.ceil(moment_time.minute() / interval) * interval
  );
};
...
export const getSelectedTime = (
  ...
  for (let i = 0; i < market_open_times.length; i++) {
    if (
      selected_time.isAfter(market_open_times[i]) &&
      selected_time.isBefore(market_close_times[i])
    ) {
