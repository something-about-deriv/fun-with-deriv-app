...
type TNonAvailableContractsList = Record<
  'contract_category' | 'contract_display_name' | 'contract_type',
  string
>[];
...
export const ContractType = (() => {
  ...
  const buildContractTypesConfig = (symbol: string): Promise<void> =>
    WS.storage
      .contractsFor(symbol)
      .then((r: Required<ContractsForSymbolResponse>) => {
        ...
        r.contracts_for.available.forEach((contract) => {
            ...
            const sub_cats =
              available_categories[
                Object.keys(available_categories).find(
                  (key) =>
                    available_categories[key].categories.indexOf(type) !== -1
                ) ?? ''
              ].categories;
          ...
          config.durations = config.hide_duration
            ? undefined
            : buildDurationConfig(contract, config.durations);
          config.trade_types = buildTradeTypesConfig(
            contract,
            config.trade_types
          );
          // @ts-expect-error: TODO: check if TS error is gone after <Duration /> is migrated to TS
          config.barriers = buildBarriersConfig(contract, config.barriers);
          config.barrier_choices =
            contract.barrier_choices as TConfig['barrier_choices'];
          config.forward_starting_dates = buildForwardStartingConfig(
            contract,
            config.forward_starting_dates
          );
          config.growth_rate_range =
            contract.growth_rate_range as TConfig['growth_rate_range'];
          config.multiplier_range =
            contract.multiplier_range as TConfig['multiplier_range'];
          config.cancellation_range =
            contract.cancellation_range as TConfig['cancellation_range'];
        ...
        const getCategories = (key = ''): string[] =>
          mutable_contracts_config[key]?.categories ?? [];
        const non_available_contracts = r.contracts_for
          .non_available as TNonAvailableContractsList;
        ...
        if (non_available_contracts) {
          non_available_contracts.forEach(({ contract_type }) => {
            ...
            const key = Object.keys(mutable_contracts_config).find((key) =>
              getCategories(key).includes(type)
            );
            const categories: Array<string | TTextValueStrings> =
              getCategories(key);
            const title = contract_types[type]?.title;
            const is_available = !!available_categories[
              key as keyof TTradeTypesCategories
            ]?.categories?.find(
        ...
        non_available_categories = getCleanedUpCategories(
          non_available_categories
        );
      });
  ...
  const getContractValues = (
    store: TTradeStore
  ): TContractValues | Record<string, never> => {
    const {
    ...
    let stored_barriers_data: TTradeStore[
      | 'short_barriers'
      | 'long_barriers'
      | 'strike_price_choices'];
    ...
    const obj_barrier = getBarriers(
      contract_type,
      contract_expiry_type,
      stored_barriers_data?.barrier
    );
    const obj_duration_unit = getDurationUnit(
      duration_unit,
      contract_type,
      obj_start_type.contract_start_type
    );

    const obj_duration_units_list = getDurationUnitsList(
      contract_type,
      obj_start_type.contract_start_type
    );
    const obj_duration_units_min_max = getDurationMinMax(
      contract_type,
      obj_start_type.contract_start_type
    );
    const obj_accumulator_range_list = getAccumulatorRange(contract_type);
    const obj_barrier_choices = getBarrierChoices(
      contract_type,
      stored_barriers_data?.barrier_choices
    );
    const obj_multiplier_range_list = getMultiplierRange(
      contract_type,
      multiplier
    );
    const obj_cancellation = getCancellation(
      contract_type,
      cancellation_duration
    );
    const obj_expiry_type = getExpiryType(
      obj_duration_units_list.duration_units_list,
      expiry_type
    );
  ...
  const getContractType = (
    ...
    const arr_list: string[] = Object.keys(list || {})
      .reduce<string[]>(
        (k, l) => [
          ...k,
          ...(list[l].categories as TTextValueStrings[]).map((ct) => ct.value),
        ],
        []
      )
    ...
    if (contract_expiry_type) {
      duration_min_max =
        'contract_expiry_type' in duration_min_max
          ? (
              duration_min_max as unknown as {
                [key: string]: TTradeStore['duration_min_max'];
              }
            )[contract_expiry_type]
          : {};
    }
  ...
  const getStartDates = (contract_type: string, current_start_date: number) => {
    const config: TConfig = getPropertyValue(available_contract_types, [
      contract_type,
      'config',
    ]);
  ...
  const getSessions = (contract_type: string, start_date: number) => {
    const config: TConfig =
      ...
      {};
    const sessions = config.forward_starting_dates?.find(
  ...
  const getValidTime = (
    ...
    if (sessions && !isSessionAvailable(sessions, compare_moment)) {
      ...
      compare_moment.minute(
        Number(
          minutes.find((m) =>
            isSessionAvailable(sessions, compare_moment.minute(+m))
  ...
  const buildMoment = (date: string | number | null, time?: string | null) => {
    const [hour, minute] = isTimeValid(time ?? '')
      ? time?.split(':') ?? []
      : [0, 0];
  ...
  const getStartTime = (
    ...
    start_time: start_date
      ? getValidTime(sessions, buildMoment(start_date, start_time))
      : null,
  });

  const getTradingEvents = async (
    date: string,
    ...
    if (!(date in trading_events)) {
      const trading_times_response: TradingTimesResponse =
        await WS.tradingTimes(date);
      const trading_times_data =
        trading_times_response.trading_times as TradingTimes;
      if (
        getPropertyValue(trading_times_response, ['trading_times', 'markets'])
      ) {
            ...
            for (let j = 0; j < submarkets.length; j++) {
              ...
              if (symbols) {
                for (let k = 0; k < symbols.length; k++) {
                  ...
                  if (
                    !trading_events[
                      trading_times_response.echo_req.trading_times as string
                    ]
                  ) {
                    trading_events[
                      trading_times_response.echo_req.trading_times as string
                  ...
                  trading_events[
                    trading_times_response.echo_req.trading_times as string
                  ][symbol.symbol] = symbol.events as TEvents;
                }
              }
            }
          }
  ...
  const getExpiryType = (
    duration_units_list: ReturnType<
      typeof getDurationUnitsList
    >['duration_units_list'],
  ...
  const getExpiryDate = (
    ...
    if (expiry_type === 'endtime') {
      ...
      if (!hasIntradayDurationUnit(duration_units_list)) {
        ...
        proper_expiry_date = (
          is_invalid ? moment_start.clone().add(1, 'day') : moment_expiry
        ).format('YYYY-MM-DD');
      } else {
        ...
        const is_invalid =
          ...
          (start_date &&
            moment_expiry.isAfter(moment_start.clone().add(1, 'day')));
        proper_expiry_date = (is_invalid ? moment_start : moment_expiry).format(
          'YYYY-MM-DD'
        );
      }
    }
  ...
  const getExpiryTime = (
    ...
    if (expiry_type === 'endtime') {
      ...
      if (
        Array.isArray(market_close_times) &&
        market_close_times?.length &&
        market_close_times[0] !== '--'
      ) {
      ...
      if (
        !start_date &&
        ServerTime.get().isBefore(buildMoment(expiry_date), 'day')
      ) {
        ...
        if (
          end_moment.isSameOrBefore(start_moment) ||
          end_moment.diff(start_moment, 'minute') < 5
        ) {
          const is_end_of_day =
            start_moment.get('hours') === 23 &&
            start_moment.get('minute') >= 55;
          const is_end_of_session =
            sessions &&
            !isSessionAvailable(
              sessions,
              start_moment.clone().add(5, 'minutes')
            );
          end_time = start_moment
            .clone()
            .add(is_end_of_day || is_end_of_session ? 0 : 5, 'minutes');
          // Set the end_time to be multiple of 5 to be equal as the SELECTED_TIME that shown to the client.
          end_time = setMinuteMultipleByFive(end_time as moment.Moment).format(
            'HH:mm'
          );
        }
  ...
  const getTradeTypes = (contract_type: string) => ({
    trade_types: getPropertyValue(available_contract_types, [
      contract_type,
      'config',
      'trade_types',
    ]),
  ...
  const getBarriers = (
    ...
    return {
      // @ts-expect-error: TODO: check if TS error is gone
  ...
  const getBasis = (contract_type: string, basis: string) => {
    const arr_basis: string[] =
      getPropertyValue(available_contract_types, [contract_type, 'basis']) ||
      [];
    ...
    const basis_list = arr_basis.reduce<TTextValueStrings[]>(
      (cur, bas) => [
        ...cur,
        {
          text: localized_basis[bas as keyof typeof localized_basis],
          value: bas,
        },
      ],
      []
  ...
  const getBarrierChoices = (
    contract_type: string,
    stored_barrier_choices = [] as string[]
  ) => ({
  ...
  const getMultiplierRange = (contract_type: string, multiplier: number) => {
    ...
    return {
      multiplier_range_list: arr_multiplier.map((m) => ({
        text: `x${m}`,
        value: m,
      })),
  ...
  const getCancellation = (
    ...
    const getText = (str: string) => {
      ...
      const name =
        'name_plural' in unit_names ? unit_names.name_plural : unit_names.name;
      return `${duration} ${name}`;
    };
    const mapCancellationRangeList = (d: string) => ({
      text: `${getText(d)}`,
      value: d,
    });
    ...
    return {
      cancellation_duration: getArrayDefaultValue(
        arr_cancellation_range,
        cancellation_duration
      ),
      cancellation_range_list: arr_cancellation_range.map(
        mapCancellationRangeList
      ),
      cached_multiplier_cancellation_list: cached_multipliers_cancellation.map(
        mapCancellationRangeList
      ),
