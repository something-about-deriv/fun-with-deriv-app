...
export const getUpdatedTicksHistoryStats = ({
  ...
  if (new_ticks_history_stats.length > 1) {
    ...
    new_ticks_history_stats[0] <= previous_history[0] &&
    last_tick_epoch > previous_epoch
  ) {
    ticks_stayed_in = [
      new_ticks_history_stats[0],
      ...previous_history.slice(0, previous_history.length - 1),
    ];
