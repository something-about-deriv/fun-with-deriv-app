...
export const getProposalErrorField = (response: PriceProposalResponse) => {
  const error_field: string = getPropertyValue(response, [
    'error',
    'details',
    'field',
  ]);
...
export const getProposalInfo = (
  ...
  const contract_basis: TObjContractBasis | undefined =
    store.is_vanilla || store.is_turbos
      ...
      : basis_list.find((o) => o.value !== store.basis) ||
        ({} as TObjContractBasis);

  const is_stake = contract_basis?.value === 'stake';

  const price = is_stake
    ? stake
    : (proposal[contract_basis?.value as keyof Proposal] as string | number);
  ...
  if (
    !obj_prev_contract_basis.value ||
    price === obj_prev_contract_basis.value
  ) {
    has_increased = !!null;
  }
...
export const createProposalRequests = (store: TTradeStore) => {
  const requests = {} as Record<
    string,
    ReturnType<typeof createProposalRequestForContract>
  >;
...
const setProposalMultiplier = (
  store: TTradeStore,
  obj_multiplier: TObjMultiplier
) => {
  obj_multiplier.multiplier = store.multiplier;
  obj_multiplier.cancellation = store.has_cancellation
    ? store.cancellation_duration
    : undefined;

  obj_multiplier.limit_order =
    store.has_take_profit || store.has_stop_loss ? {} : undefined;

  if (
    store.has_take_profit &&
    store.take_profit &&
    obj_multiplier.limit_order
  ) {
...
const setProposalAccumulator = (
  store: TTradeStore,
  obj_accumulator: TObjAccum
) => {
...
const createProposalRequestForContract = (
  ...
  if (store.expiry_type === 'endtime' && store.expiry_time) {
    ...
    obj_expiry.date_expiry = convertToUnix(
      expiry_date.unix(),
      store.expiry_time
    );
  }
  ...
  if (
    isTurbosContract(store.contract_type) &&
    store.has_take_profit &&
    store.take_profit
  ) {
  ...
  return {
    ...
    ...((store.barrier_count > 0 ||
      store.form_components.indexOf('last_digit') !== -1) &&
