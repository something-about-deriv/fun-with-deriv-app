...
const isBeforeDate = (
  ...
  should_only_check_hour: boolean
) => {
  ...
  return (
    compare_moment.isBefore(now_moment) &&
    now_moment.unix() !== compare_moment.unix()
  );
};
...
export const isSessionAvailable = (
  ...
  (!sessions.length ||
    !!sessions.find((session) =>
      compare_moment.isBetween(
        session.open,
        session.close,
        should_only_check_hour ? 'hour' : null,
        '[]'
      )
    ));
