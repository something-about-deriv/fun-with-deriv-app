...
const tradeSpecificBarrierCheck = (is_vanilla: boolean, input: number) =>
  is_vanilla || input !== 0;
    ...
    rules: [
      [
        'req',
        {
          condition: (store) =>
            !!store.barrier_count &&
            store.form_components.indexOf('barrier') > -1,
      ...
      [
        'custom',
        {
          func: (value: TTradeStore['barrier_1'], options, store, inputs) =>
            Number(store?.barrier_count) > 1
              ? +value > Number(inputs?.barrier_2)
              : true,
          message: localize(
        ...
        'custom',
        {
          func: (value: TTradeStore['barrier_1'], options, store, inputs) =>
            /^[+-]/.test(inputs?.barrier_1 ?? '')
              ? tradeSpecificBarrierCheck(
                  !!store?.is_vanilla,
                  Number(inputs?.barrier_1)
                )
              : true,
      ...
      [
        'req',
        {
          condition: (store: TTradeStore) =>
            store.has_stop_loss && !store.stop_loss,
  ...
  take_profit: {
    rules: [
      [
        ...
        {
          condition: (store: TTradeStore) =>
            store.has_take_profit && !store.take_profit,
