...
class Validator {
  ...
  addFailure(
    attribute: string,
    rule: { name: string; options: TRuleOptions },
  ...
  check() {
    Object.keys(this.input).forEach((attribute) => {
      ...
      (this.rules as unknown as { [key: string]: Array<TRule> })[
        attribute
      ].forEach((rule: TRule) => {
        ...
        if (
          ...
          typeof ruleObject.validator !== 'function'
        ) {
          return;
        }

        if (
          ruleObject.options.condition &&
          !ruleObject.options.condition(this.store)
        ) {
          return;
        }
  ...
  static getRuleObject(rule: TRule) {
    ...
    const rule_object_options = (
      is_rule_string ? {} : rule[1] || {}
    ) as TRuleOptions;
    return {
