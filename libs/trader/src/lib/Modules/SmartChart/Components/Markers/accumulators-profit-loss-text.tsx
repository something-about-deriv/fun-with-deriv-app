...
const AccumulatorsProfitLossText = ({
  ...
  const [current_profit_tenth, setCurrentProfitTenth] =
    React.useState(prev_profit_tenth);
  ...
  const runThroughTenthDigit = (
    ...
    const interval_id = setInterval(() => {
      if (action === ACTIONS.INC && profit_tenth_ref.current < end) {
        ...
        action === ACTIONS.ADD10 &&
        profit_tenth_ref.current < start + 10
      ) {
        profit_tenth_ref.current += 1;
      } else if (
        action === ACTIONS.ADD10
          ? profit_tenth_ref.current === start + 10
          : profit_tenth_ref.current === end
      ) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className={is_sliding ? `${className}__sliding-tenth` : ''}>
          {current_profit_tenth}
        </div>
        {`${profit_hundredths}`}
      </Text>
      <Text
