...
type TContractInfo = ReturnType<
  typeof useStore
>['portfolio']['all_positions'][number]['contract_info'];
...
const AccumulatorsProfitLossTooltip = ({
  ...
  const should_show_profit_percentage =
    getDecimalPlaces(currency) > 2 && !!profit_percentage;
  ...
  if (!is_sold && current_spot_time && high_barrier && should_show_profit_text)
    return (
      <AccumulatorsProfitLossText
        ...
        profit_value={
          should_show_profit_percentage ? profit_percentage : profit
        }
      ...
      >
        <div
          className={classNames(
            `${className}__content`,
            `arrow-${opposite_arrow_position}`
          )}
        >
          <Text
            size={isMobile() ? 'xxxxs' : 'xxs'}
