...
type TTradeParamsMobile = {
  ...
  setSelectedAmount: (
    basis: string,
    selected_basis_value: string | number
  ) => void;
  stake_value: number;
  payout_value: number;
  // duration
...
const makeGetDefaultDuration =
  (trade_duration: number, trade_duration_unit: string) =>
  (duration_unit: string) =>
    trade_duration_unit === duration_unit
      ? trade_duration
      : DEFAULT_DURATION[duration_unit as keyof typeof DEFAULT_DURATION];

const TradeParamsModal = observer(
  ({ is_open, toggleModal, tab_index }: TTradeParamsModal) => {
    ...
    const getDefaultDuration = React.useCallback(
      makeGetDefaultDuration(duration, duration_unit),
      []
    );
    ...
    const setTradeParamTabIdx = (trade_param_tab_idx: number) =>
      dispatch({ trade_param_tab_idx });

    const setDurationTabIdx = (duration_tab_idx?: number) =>
      dispatch({ duration_tab_idx });

    const setAmountTabIdx = (amount_tab_idx?: number) =>
      dispatch({ amount_tab_idx });
    ...
    const isVisible = (component_key: string): boolean =>
      form_components.includes(component_key);
    return (
      <React.Fragment>
        <Modal
...
const TradeParamsMobile = observer(
  ({
    ...
    const { basis_list, basis, expiry_epoch, is_turbos, is_vanilla } =
      useTraderStore();
    const getDurationText = () => {
      const duration = duration_units_list.find(
        (d) => d.value === duration_unit
      );
      return `${duration_value} ${
        duration &&
        (duration_value > 1
          ? localize(duration.text)
          : localize(duration.text.slice(0, -1)))
      }`;
    };
    ...
    const getAmountText = () => {
      ...
      const active_index = has_selected_tab_idx
        ? amount_tab_idx
        : basis_list.findIndex((b) => b.value === basis);

      return (
        <Money
          currency={currency}
          show_currency
          amount={active_index === 1 ? payout_value : stake_value}
        />
      );
    };
