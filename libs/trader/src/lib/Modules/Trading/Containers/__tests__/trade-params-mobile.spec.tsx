...
import TradeParamsModal, {
  LastDigitMobile,
  BarrierMobile,
} from '../trade-params-mobile';
...
jest.mock('@deriv/components', () => ({
  ...
  Modal: jest.fn(({ children, height }) => (
    <div data-testid={height}>{children}</div>
  )),
...
jest.mock('Modules/Trading/Components/Form/TradeParams/amount-mobile', () =>
  jest.fn((props) => (
    <div>
      ...
      <button onClick={() => props.setSelectedAmount('stake', 20)}>
        setSelectedAmount
      </button>
...
jest.mock(
  'Modules/Trading/Components/Form/TradeParams/Duration/duration-mobile',
  () =>
    jest.fn((props) => (
      <div>
        <p>DurationMobile</p>
        <p>{props.has_amount_error && 'Amount error'}</p>
        <button onClick={() => props.setDurationError(true)}>
          setDurationError
        </button>
      </div>
    ))
...
describe('<TradeParamsModal />', () => {
  ...
  it('should not render children components if the form_components is an empty array', () => {
    ...
    render(
      mockTradeParamsModal(mockStore(default_mock_store), mock_default_props)
    );
...
describe('<LastDigitMobile />', () => {
  ...
  it('should not render LastDigit component if it is not in the form_components array', () => {
    const { container } = render(
      mockLastDigitMobile(mockStore(default_mock_store))
    );
