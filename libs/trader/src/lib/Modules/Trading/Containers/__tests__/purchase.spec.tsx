...
const default_mock_store = {
  modules: {
    trade: {
      ...
      proposal_info: {
        CALL: { id: '123', message: 'test_message', has_error: true },
      },
      purchase_info: {},
      symbol: 'test_symbol',
      validation_errors: {},
      vanilla_trade_type: 'VANILLALONGCALL',
      trade_types: { CALL: 'Higher', PUT: 'Lower' } as {
        [key: string]: string;
      },
...
type TNewMockedProps = typeof default_mock_store &
  Partial<{
    portfolio: {
      active_positions: { contract_info: TContractInfo; type: string }[];
    };
  }>;
...
jest.mock(
  'Modules/Trading/Components/Elements/purchase-buttons-overlay.jsx',
  () => jest.fn(() => <div>PurchaseButtonsOverlay component</div>)
);
jest.mock(
  'Modules/Trading/Components/Form/TradeParams/Accumulator/accumulators-sell-button',
  ...
  it('should render Sell button if accumulator contract is already in active positions', () => {
    const new_mocked_store: TNewMockedProps = {
      ...
      portfolio: {
        active_positions: [
          {
            contract_info: mockContractInfo({ underlying: 'test_symbol' }),
            type: 'accumulator',
          },
        ],
      },
    ...
    expect(
      screen.queryByText(/PurchaseField component/i)
