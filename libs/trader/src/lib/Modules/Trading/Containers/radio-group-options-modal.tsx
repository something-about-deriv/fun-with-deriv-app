...
const RadioGroupOptionsModal = observer(
  ({ is_open, modal_title, toggleModal }: TRadioGroupOptionsModal) => {
    const {
    ...
    const has_error_or_not_loaded =
      proposal_info?.ACCU?.has_error ||
      !proposal_info?.ACCU?.id ||
      isEmptyObject(proposal_info);
              ...
              <RadioGroupWithInfoMobile
                ...
                info={localize(
                  'Your stake will grow at {{growth_rate}}% per tick as long as the current spot price remains within ±{{tick_size_barrier}} from the previous spot price.',
                  {
                    ...
                    tick_size_barrier:
                      getTickSizeBarrierPercentage(tick_size_barrier),
                  }
                )}
