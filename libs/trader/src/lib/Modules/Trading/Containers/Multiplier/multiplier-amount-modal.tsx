...
const MultiplierAmountModal = ({
  is_open,
  toggleModal,
}: TMultiplierAmountModal) => {
...
const TradeParamsMobile = observer(
  ({ toggleModal }: { toggleModal: TToggleModal }) => {
    const trade_store = useTraderStore();
    const { amount } = trade_store;

    const [stake_value, setStakeValue] = React.useState<string | number>(
      amount
    );
    ...
    React.useEffect(() => {
      ...
      const onProposalResponse: TTradeStore['onProposalResponse'] = (
        response
      ) => {
        ...
        if (
          ...
          proposal.limit_order?.stop_out &&
            setStopOut(proposal.limit_order.stop_out?.order_amount);
        } else if (subscription?.id) {
          WS.forget(subscription.id);
        }
      };
      const dispose = requestPreviewProposal(trade_store, onProposalResponse, {
        amount: stake_value,
      });
