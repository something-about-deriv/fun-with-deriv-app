...
const RiskManagementDialog = observer(
  ({ is_open, onClose, toggleDialog }: TRiskManagementDialog) => {
    const {
      is_turbos,
      take_profit,
      has_take_profit,
    ...
    const isStateUnchanged = () => {
      return isDeepEqual(
        getStateToCompare(state),
        getStateToCompare(applied_risk_management_state)
      );
    };
      ...
      setValidationErrors({
        take_profit:
          new_state.has_take_profit && !new_state.take_profit
            ...
            : [],
        stop_loss:
          new_state.has_stop_loss && !new_state.stop_loss
    ...
    const onChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      new_state[name as 'take_profit' | 'stop_loss' | 'cancellation_duration'] =
        value;
      setState(new_state);
      validate(new_state);
    };

    const onChangeMultipleLocal = (
      props: keyof typeof applied_risk_management_state
    ) => {
      const new_state = { ...state };
        ...
        >
          ...
          >
            <TakeProfit
              ...
              onChange={
                onChange as React.ComponentProps<typeof TakeProfit>['onChange']
              }
              onChangeMultiple={
            ...
            {!is_turbos && (
              <StopLoss
                ...
                onChange={
                  onChange as React.ComponentProps<typeof StopLoss>['onChange']
                }
                onChangeMultiple={
                  onChangeMultipleLocal as unknown as React.ComponentProps<
                    typeof StopLoss
