...
const Trade = observer(() => {
  ...
  const form_wrapper_class = is_mobile
    ? 'mobile-wrapper'
    : 'sidebar__container desktop-only';
    ...
    <React.Fragment>
      ...
      <div
        ...
        className={classNames('trade-container', {
          [`trade-container--${is_accumulator ? 'accumulators' : 'turbos'}`]:
            is_accumulator || is_turbos,
        })}
      >
        <DesktopWrapper>
          <PositionsDrawer />
          ...
          <React.Suspense
            fallback={
              <ChartLoader
                ...
                is_visible={!symbol || is_chart_loading}
              />
            }
          >
            <DesktopWrapper>
              ...
              >
                <ChartLoader
                  is_visible={
                    is_chart_loading || should_show_active_symbols_loading
                  }
                />
                <ChartTrade
            ...
            {!(open_launch_modal && is_logged_in && !is_already_shown) && (
              <MobileWrapper>
                ...
                >
                  {show_digits_stats && (
                    <DigitsWidget digits={digits} tick={tick} />
                  )}
                  <ChartTrade
                    topWidgets={topWidgets}
                    charts_ref={charts_ref}
                    bottomWidgets={
                      show_digits_stats ? bottomWidgets : undefined
                    }
                    is_accumulator={is_accumulator}
...
const ChartTrade = observer((props) => {
  const { is_accumulator, has_barrier, end_epoch, topWidgets, charts_ref } =
    props;
  ...
  const settings = {
    ...
    ...(is_accumulator
      ? { whitespace: 190, minimumLeftBars: is_mobile ? 3 : undefined }
      : {}),
  ...
  const bottomWidgets = React.useCallback(
    ({ digits, tick }) => (
      <ChartBottomWidgets
        digits={digits}
        tick={tick}
        show_accumulators_stats={is_accumulator}
        is_trade_page
      />
    ),
    [is_accumulator]
  ...
  const getMarketsOrder = (active_symbols) => {
    ...
    const has_synthetic_index = !!active_symbols.find(
  ...
  return (
    <SmartChartWithRef
      ...
      bottomWidgets={
        (is_accumulator || show_digits_stats) && isDesktop()
          ? bottomWidgets
          : props.bottomWidgets
      }
