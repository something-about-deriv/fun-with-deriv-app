...
const Purchase = observer(
  ({ is_market_closed }: { is_market_closed?: boolean }) => {
    const {
    ...
    const isLoading = (
      info: TTradeStore['proposal_info'][string] | Record<string, never>
    ) => {
      const has_validation_error = Object.values(validation_errors).some(
        (e) => e.length
      );
    ...
    const active_accu_contract = is_accumulator
      ? all_positions.find(
          ({ contract_info, type }) =>
            ...
            contract_info.underlying === symbol &&
    ...
    const indicative =
      (is_valid_to_sell &&
        active_accu_contract &&
        getIndicativePrice(active_accu_contract.contract_info)) ||
      null;
