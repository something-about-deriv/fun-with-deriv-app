  ...
  return (
    <React.Fragment>
      ...
      {(isVisible('trade_type_tabs') ||
        ...
        <Fieldset
          className={classNames(
            ...
            'trade-container__fieldset--no-padding'
          )}
        >
          {isVisible('trade_type_tabs') && (
            <TradeTypeTabs key={'trade_type_tabs'} />
          )}
          {isVisible('strike') && <Strike key={'strike'} />}
          {isVisible('barrier_selector') && (
            <BarrierSelector key={'barrier_selector'} />
          )}
        </Fieldset>
      )}
      {isVisible('amount') && (
      ...
      {isVisible('accu_info_display') && (
        <AccumulatorsInfoDisplay key={'accu_info_display'} />
      )}
    </React.Fragment>
  );
});
