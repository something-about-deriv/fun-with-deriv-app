...
const Strike = observer(() => {
  ...
  const is_24_hours_contract = expiry_date
    ? toMoment(expiry_date).isSame(toMoment(server_time), 'day')
    : false;
  ...
  const strike_price_list = strike_price_choices.map(
    (strike_price: string) => ({
      text: strike_price,
      value: strike_price,
    })
