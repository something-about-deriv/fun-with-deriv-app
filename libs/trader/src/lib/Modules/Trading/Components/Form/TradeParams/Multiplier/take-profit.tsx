...
const TakeProfit = observer((props: TTakeProfitProps) => {
  ...
  const { is_accumulator, is_multiplier, currency, has_open_accu_contract } =
    trade;
  ...
  const changeValue = (
    e: Parameters<React.ComponentProps<typeof InputWithCheckbox>['onChange']>[0]
  ) => {
    ...
    <Fieldset className="trade-container__fieldset">
      <InputWithCheckbox
        ...
        className={classNames({
          'trade-container__amount trade-container__amount--multipliers':
            isDesktop(),
        })}
        currency={currency}
        ...
        error_messages={
          has_take_profit ? validation_errors?.take_profit : undefined
        }
