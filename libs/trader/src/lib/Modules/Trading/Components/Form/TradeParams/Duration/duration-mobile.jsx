...
const Ticks = observer(
  ({
    ...
    const [min_tick, max_tick] = getDurationMinMaxValues(
      duration_min_max,
      'tick',
      't'
    );
    ...
    const tick_duration =
      trade_duration < min_tick && selected_duration < min_tick
        ? min_tick
        : selected_duration;
    return (
  ...
  ({
    ...
    const setExpiryDate = (epoch, duration) => {
      if (trade_duration_unit !== 'd') {
        return moment
          .utc()
          .add(Number(duration), 'days')
          .format('D MMM YYYY, [23]:[59]:[59] [GMT +0]');
      }
      let expiry_date = new Date(
        (epoch - trade_duration * 24 * 60 * 60) * 1000
      );
      if (duration) {
        expiry_date = new Date(
          expiry_date.getTime() + duration * 24 * 60 * 60 * 1000
        );
      }
    ...
    const fixed_date = !has_error
      ? setExpiryDate(expiry_epoch, duration_values?.d_duration)
      : '';
        ...
        >
          {is_vanilla && (
            <DurationRangeText
              min={min}
              max={max}
              duration_unit_text={duration_unit_text}
            />
          )}
          ...
          {duration_units_list.map((duration_unit_option) => {
            switch (duration_unit_option.value) {
              case 't':
                return (
                  <div
                    label={duration_unit_option.text}
                    key={duration_unit_option.value}
                  >
                    <Ticks
