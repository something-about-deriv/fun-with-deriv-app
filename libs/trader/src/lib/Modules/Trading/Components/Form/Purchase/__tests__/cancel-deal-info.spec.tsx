...
describe('<CancelDealInfo />', () => {
  const mockCancelDealInfo = (
    mocked_store: TCoreStores,
    mock_proposal_info: TProposalTypeInfo
  ) => {
    return (
      <TraderProviders store={mocked_store}>
        <CancelDealInfo
          proposal_info={mock_proposal_info}
          data_testid="cancellation-wrapper"
        />
      </TraderProviders>
    );
  };
  ...
  it('should not render component if has_cancellation is false', () => {
    const { container } = render(
      mockCancelDealInfo(mockStore(default_mock_store), mock_proposal_info)
    );
  ...
  it('should not render Money component if id is falsy and has_error is null or undefined', () => {
    mock_proposal_info.has_error =
      null as unknown as TProposalTypeInfo['has_error'];
