...
jest.mock('App/Components/Form/number-selector', () =>
  jest.fn(() => <div>{mocked_number_selector}</div>)
);
...
describe('<LastDigit />', () => {
  const mockLastDigit = (
    ...
    mocked_props?: React.ComponentProps<typeof LastDigit>
  ) => {
    return (
  ...
  it('should render only phrase about last digit if is_minimizes is true', () => {
    render(
      mockLastDigit(mockStore(default_mock_store), { is_minimized: true })
    );
