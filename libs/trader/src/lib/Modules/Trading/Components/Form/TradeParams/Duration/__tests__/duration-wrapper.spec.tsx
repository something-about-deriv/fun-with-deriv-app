...
jest.mock('../../../DatePicker', () =>
  jest.fn(() => <div>TradingDatePicker</div>)
);
jest.mock('../../../TimePicker', () =>
  jest.fn(() => <div>TradingTimePicker</div>)
);
...
describe('<DurationWrapper />', () => {
  ...
  it('should execute onChange & onChangeUiStore when a user is typing a new value', () => {
    ...
    expect(default_mock_store.ui.onChangeUiStore).toHaveBeenCalledWith({
      name: 'duration_m',
      value: 25,
    });
    expect(
      default_mock_store.modules.trade.onChangeMultiple
