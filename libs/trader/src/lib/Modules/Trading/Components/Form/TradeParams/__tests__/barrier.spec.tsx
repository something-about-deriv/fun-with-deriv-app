...
const mocked_labeled_quantity_input_mobile =
  'Mocked Labeled Quantity Input Mobile Component';
...
jest.mock('../../Purchase/value-movement', () =>
  jest.fn(() => <div>{mocked_value_movement}</div>)
);

describe('<Barrier />', () => {
  const mockBarrier = (
    mocked_store: TCoreStores,
    mocked_props: React.ComponentProps<typeof Barrier>
  ) => {
    return (
  ...
  it('should render Modal for mobile devices after user clicked on LabeledQuantityInputMobile', () => {
    ...
    userEvent.click(
      screen.getAllByText(mocked_labeled_quantity_input_mobile)[0]
    );
