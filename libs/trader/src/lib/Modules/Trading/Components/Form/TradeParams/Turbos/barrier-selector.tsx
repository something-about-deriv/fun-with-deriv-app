...
const BarrierSelector = observer(() => {
  const { barrier_1, onChange, setHoveredBarrier, barrier_choices } =
    useTraderStore();
  const [is_barriers_table_expanded, setIsBarriersTableExpanded] =
    React.useState(false);
  const [is_mobile_tooltip_visible, setIsMobileTooltipVisible] =
    React.useState(false);
  const [selected_barrier, setSelectedBarrier] = React.useState(barrier_1);

  const toggleMobileTooltip = () =>
    setIsMobileTooltipVisible(!is_mobile_tooltip_visible);
      ...
      <DesktopWrapper>
        ...
        >
          <div
            ...
            className="trade-container__barriers__wrapper"
          >
            ...
            <Text
              size="xs"
              className="trade-container__barriers-value"
              data-testid="current_barrier"
            >
              {barrier_1}
              <Icon
                icon="IcChevronLeft"
                className="trade-container__barriers-value--arrow-right"
              />
            </Text>
          </div>
        </Fieldset>
