...
const ContractTypeWidget = observer(
  ({
    ...
    const [is_dialog_open, setDialogVisibility] = React.useState<
      boolean | null
    >();
    const [is_info_dialog_open, setInfoDialogVisibility] =
      React.useState(false);
    const [selected_category, setSelectedCategory] =
      React.useState<TList['key']>('All');
    ...
    const handleCategoryClick: React.ComponentProps<
      typeof ContractType.Dialog
    >['onCategoryClick'] = ({ key }) => {
    ...
    const onChangeInput = (searchQueryItem: string) =>
      setSearchQuery(searchQueryItem);

    const list_with_category = () => {
      const ordered_list = list && getCategoriesSortedByKey(list);
      const ordered_unavailable_types_list = getCategoriesSortedByKey(
        unavailable_trade_types_list
      );
      const all_trade_types_list = ordered_list?.concat(
        ordered_unavailable_types_list
      );
      const accumulators_category = all_trade_types_list?.filter(
        ({ label }) => label === localize('Accumulators')
      );
      const multipliers_category = all_trade_types_list?.filter(
      ...
      const contract_type_category_icon: { [key: string]: string } =
        getContractTypeCategoryIcons();
      ...
      if (options_category && options_category.length > 0) {
        categories.push({
          ...
          component: options_category.some((category) =>
            /Vanillas|Turbos/i.test(category.key)
          ) ? (
            <span className="dc-vertical-tab__header--new">
              {localize('NEW')}!
            </span>
          ) : null,
          key: 'Options',
      ...
      return categories.map((contract_category) => {
        const contract_types = contract_category?.contract_categories?.reduce<
          TContractType[]
    ...
    const selected_category_contracts = () => {
      ...
      return (selected_list_category || list_with_category()[0])
        .contract_categories;
    };
    const should_show_info_banner = !!selected_category_contracts()?.some(
      (i) => i.is_unavailable
    );
              ...
              <InlineMessage
                ...
                message={
                  <Localize
                    ...
                    values={{
                      symbol: active_symbols.find((s) => s.symbol === symbol)
                        ?.display_name,
                    }}
                    shouldUnescape
                  />
