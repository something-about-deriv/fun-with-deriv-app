...
const default_mock_store = {
  modules: {
    trade: {
      ...
      contract_types_list: {} as ReturnType<
      ...
      stake_boundary: {
        [TURBOS.LONG.toUpperCase()]: { min_stake: 1, max_stake: 10000 },
      } as ReturnType<typeof useTraderStore>['stake_boundary'],
      onChange: jest.fn(),
      validation_errors: {} as ReturnType<
        typeof useTraderStore
      >['validation_errors'],
    },
  },
...
jest.mock('../Multiplier/multiplier', () =>
  jest.fn(() => <div>{mocked_multiplier}</div>)
);
jest.mock('../Multiplier/info', () =>
  jest.fn(() => <div>{mocked_multiplier_info}</div>)
);
jest.mock('../min-max-stake-info', () =>
  jest.fn(() => <div>{mocked_min_max_stake_info}</div>)
);
...
describe('<Amount />', () => {
  const mockAmount = (
    ...
    mocked_props?: React.ComponentProps<typeof Amount>
  ) => {
    return (
  ...
  it('should render only specific for Multipliers components if it is a Multiplier contract type', () => {
    ...
    expect(
      screen.queryByText(mocked_min_max_stake_info)
