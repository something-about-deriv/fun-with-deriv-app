...
const Duration = ({
  ...
  React.useEffect(() => {
    if (isVanillaContract(contract_type)) {
      onToggleDurationType({
        target: { value: true, name: 'is_advanced_duration' },
      });
    }
  }, [contract_type]);
  ...
  if (is_minimized) {
    ...
    const duration_unit_text = (
      duration_units_list.find((o) => o.value === duration_unit) || {}
    ).text;
    return (
  ...
  const changeDurationValue = ({ target }) => {
    const { name, value } = target;
    const duration_name = `duration_${
      is_advanced_duration ? advanced_duration_unit : simple_duration_unit
    }`;
  ...
  const onToggleDurationType = ({ target }) => {
    ...
    let current_duration_unit = is_advanced
      ? advanced_duration_unit
      : simple_duration_unit;
    ...
    const has_same_duration =
      current_duration_unit === duration_unit && duration_value === duration;
    ...
    const should_update_trade_store = Object.keys(
