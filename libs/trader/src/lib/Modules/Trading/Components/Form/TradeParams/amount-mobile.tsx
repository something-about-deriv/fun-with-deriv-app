...
const Basis = observer(
  ({
    ...
    const formatAmount = (value: number | string) =>
      !isNaN(+value) && value !== ''
        ? Number(value).toFixed(user_currency_decimal_places)
        : value;
    const setBasisAndAmount = (amount: number | string) => {
      ...
      if (duration_value !== trade_duration && !has_duration_error)
    ...
    const validateAmount = (value: number | string) => {
      const localized_message = (
        <Localize i18n_default_text="Should not be 0 or empty" />
      );
      ...
      if (value.toString() === '0.' || selected_value === 0) {
        ...
        isNaN(selected_value) ||
        selected_value < min_amount ||
    ...
    if (basis_list.length === 1) {
      return (
        <Basis
          ...
          selected_basis={
            basis_list[0].value === 'stake' ? stake_value : payout_value
          }
