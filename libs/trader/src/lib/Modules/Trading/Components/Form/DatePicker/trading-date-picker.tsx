...
// import { useTraderStore } from 'Stores/useTraderStores';
// import { TEvents } from '@deriv/analytics';
...
const TradingDatePicker = observer(
  ({ id, is_24_hours_contract, mode, name }: TTradingDatePickerProps) => {
    ...
    const getMomentContractStartDateTime = () => {
      return setTime(
        ...
        isTimeValid(start_time ?? '')
          ? start_time
          : server_time?.format('HH:mm:ss') ?? ''
      );
    };

    const getMaxDailyDuration = () => {
      return duration_min_max.daily
        ? duration_min_max.daily.max
        : 365 * 24 * 3600;
    };
    ...
    const getMaxDateDuration = () => {
      ...
      return is_24_hours_contract
        ? getMomentContractStartDateTime()
            .clone()
            .add(max_daily_duration, 'second')
        : getMomentContractStartDateTime()
            .clone()
            .add(getMaxDailyDuration(), 'second');
    };
    ...
    const getFooter = () => {
      ...
      if (+duration === 1)
        return localize('Duration: {{duration}} day', { duration });
    ...
    const onChangeDate: TDatePickerOnChange = (e) => {
      ...
      if (typeof onChange === 'function' && e.target) {
        onChange({
          target: {
            name: e.target.name || '',
            value: hasRangeSelection()
              ? e.target.value
              : toMoment(e.target.value).format('YYYY-MM-DD'),
          },
        });
      }
    };
    ...
    const onChangeCalendarMonth = React.useCallback(
      ...
      async (e = toMoment().format('YYYY-MM-DD')) => {
        ...
        const events:
          ...
          | [] = await ContractType.getTradingEvents(e, symbol);
