...
const AccumulatorsAmountMobile = observer(
  ({ is_nativepicker }: TAccumulatorsAmountMobile) => {
    const { ui, client } = useStore();
    const { current_focus, setCurrentFocus } = ui;
    const { is_single_currency } = client;
    const { amount, currency, onChange, has_open_accu_contract } =
      useTraderStore();
    return (
      <>
        <MobileWrapper>
          <LabeledQuantityInputMobile
            ...
            className={classNames(
              'trade-container__amount',
              'trade-container__amount--accumulator'
            )}
            classNameInlinePrefix="trade-container__currency"
            classNameInput={classNames(
              'trade-container__input',
              'trade-container__input--accumulator'
            )}
            currency={currency}
