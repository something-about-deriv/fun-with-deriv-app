...
const DurationWrapper = observer(() => {
  ...
  const hasDurationUnit = (duration_type, is_advanced) => {
    ...
    if (duration_list.length > 1 && !is_advanced) {
      duration_list = duration_list.filter(
        (du) => du.value === 'm' || du.value === 't'
      );
    }
  ...
  const current_duration_unit = is_advanced_duration
    ? advanced_duration_unit
    : simple_duration_unit;
  const has_missing_duration_unit = !hasDurationUnit(
  ...
  const assertDurationIsWithinBoundary = React.useCallback(
    (current_duration) => {
      ...
      if (
        !(current_duration < min_value) &&
        current_duration > max_value &&
        duration_unit !== 'd'
      ) {
        onChangeUiStore({
          name: `duration_${duration_unit}`,
  ...
  React.useEffect(() => {
    if (is_advanced_duration && expiry_type !== advanced_expiry_type) {
      onChange({
        target: { name: 'expiry_type', value: advanced_expiry_type },
      });
    }
  ...
  React.useEffect(() => {
    ...
    const simple_is_not_type_duration =
      !is_advanced_duration && expiry_type !== 'duration';
  ...
  return (
    <Duration
      hasDurationUnit={hasDurationUnit}
      max_value={max_value}
      min_value={min_value}
      {...duration_props}
    />
