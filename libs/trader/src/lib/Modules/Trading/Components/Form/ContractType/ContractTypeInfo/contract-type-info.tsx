...
const Info = observer(({ handleSelect, item, list }: TInfo) => {
  ...
  const [selected_tab, setSelectedTab] = React.useState<TSelectedTab>(
    TABS.DESCRIPTION
  );
  const contract_types: TContractType[] | undefined = getContractTypes(
    ...
    (i: { value: TContractType['value'] }) =>
      i.value !== 'rise_fall_equal' &&
      i.value !== TURBOS.SHORT &&
      i.value !== VANILLALONG.PUT
  );
  const has_toggle_buttons = /accumulator|turboslong|vanilla|multiplier/i.test(
    item.value
  );
      ...
      <div key={type.value} className="contract-type-info__card">
        <ThemedScrollbars
          className={classNames('contract-type-info__scrollbars', {
            'contract-type-info__scrollbars-description--active':
          ...
          <div
            className={classNames('contract-type-info__content', {
              ...
              'contract-type-info__gif--has-video':
                should_show_video && is_description_tab_selected,
            })}
          >
            {is_description_tab_selected ? (
              <React.Fragment>
                ...
                <TradeCategories
                  ...
                  is_multiplier_fx={
                    !cached_multiplier_cancellation_list?.length
                  }
                />
              </React.Fragment>
            ) : (
