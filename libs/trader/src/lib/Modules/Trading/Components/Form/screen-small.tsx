...
import {
  BarrierMobile,
  LastDigitMobile,
} from '../../../../Modules/Trading/Containers/trade-params-mobile';
...
// import '../../../../app/_common/mobile-widget.scss';
...
const CollapsibleTradeParams = ({
  ...
  React.useEffect(() => {
    if (previous_symbol && is_allow_equal && has_allow_equals)
    ...
    <Collapsible position="top" is_collapsed={is_collapsed} onClick={onClick}>
      ...
      <div className="trade-params__contract-type-container">
        ...
        {isVisible('last_digit') && is_collapsed && (
          ...
          >
            <Localize
              i18n_default_text="Digit: {{last_digit}} "
              values={{ last_digit }}
            />
          </Text>
        )}
      ...
      <div
        className={classNames({
          ...
          [`purchase-container__${is_accumulator ? 'accumulator' : 'turbos'}`]:
...
const ScreenSmall = observer(
  ({ is_trade_enabled }: { is_trade_enabled: boolean }) => {
    ...
    const has_allow_equals =
      isRiseFallEqual(contract_type) &&
      (has_callputequal_duration || expiry_type === 'endtime') &&
      has_callputequal;
    ...
    return !is_trade_enabled ? (
      ...
      <CollapsibleTradeParams
        has_allow_equals={has_allow_equals}
        {...collapsible_trade_params_props}
      />
    );
  }
);
