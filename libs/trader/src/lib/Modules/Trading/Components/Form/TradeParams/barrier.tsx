...
const Barrier = observer(({ is_minimized, is_absolute_only }: TBarrier) => {
  ...
  const type_with_current_spot = Object.keys(trade_types).find(
    (type) => proposal_info?.[type]?.spot
  );
  let contract_info, has_spot_increased;
  if (type_with_current_spot)
    contract_info = proposal_info?.[type_with_current_spot];
  ...
  const barrier_title =
    barrier_count === 1 ? localize('Barrier') : localize('Barriers');
  const has_error_or_not_loaded =
    contract_info?.has_error || !contract_info?.id;
