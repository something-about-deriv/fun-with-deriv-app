...
const RiskManagementInfo = observer(() => {
  ...
  const has_risk_management =
    has_take_profit || has_stop_loss || has_cancellation;
  const toggleDialog = () => {
    setDialogOpen(!is_dialog_open);
  };
  const cancellation_label = cancellation_range_list.find(
    (r) => r.value === cancellation_duration
  )?.text;
