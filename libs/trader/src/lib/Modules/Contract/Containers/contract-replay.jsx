...
const ContractReplay = observer(({ contract_id }) => {
  ...
  const trade_type_feature_flag =
    ...
    getContractTypeFeatureFlag(
      ...
      isHighLow(contract_info)
    );
  const is_trade_type_disabled =
    useFeatureFlags()[`is_${trade_type_feature_flag}_enabled`] === false;
  ...
  const onClickClose = React.useCallback(() => {
    setIsVisible(false);
    const is_from_table_row = !isEmptyObject(location.state)
      ? location.state.from_table_row
      : false;
  ...
  const is_smarttrader_contract = isSmartTraderContract(
    contract_info.contract_type
  );
          ...
          <MobileWrapper>
            <div
              className={classNames('contract-drawer__mobile-wrapper', {
                'contract-drawer__mobile-wrapper--is-multiplier':
                  isMobile() && is_multiplier,
              })}
            >
              {contract_drawer_el}
            </div>
          ...
          <React.Suspense fallback={<div />}>
            <div
              className={classNames('replay-chart__container', {
                'replay-chart__container--is-multiplier':
                  isMobile() && is_multiplier,
...
const ReplayChart = observer(({ is_accumulator_contract }) => {
  ...
  const { contract_store, chart_state, chartStateChange, margin } =
    contract_replay;
  const {
    contract_config,
    is_digit_contract,
  ...
  const allow_scroll_to_epoch =
    chart_state === 'READY' || chart_state === 'SCROLL_TO_LEFT';
  ...
  const scroll_to_epoch = allow_scroll_to_epoch
