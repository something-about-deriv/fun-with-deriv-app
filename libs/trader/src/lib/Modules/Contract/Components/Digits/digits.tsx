...
type TTickStream = NonNullable<
  TContractStore['contract_info']['tick_stream']
>[number];
type TTickData =
  | TTickSpotData
  | null
...
const DigitsWrapper = ({
  ...
  const is_contract_elapsed = is_trade_page
    ? isContractElapsed(contract_info, tick)
    : false;
  ...
  React.useEffect(() => {
    if (onChangeStatus) {
      onChangeStatus({
        status,
        current_tick: tick && 'current_tick' in tick ? tick.current_tick : null,
      });
    }
  ...
  return (
    <LastDigitPrediction
      ...
      barrier={
        !is_contract_elapsed && is_tick_ready
          ? Number(contract_info.barrier)
          : null
      }
      contract_type={
        !is_contract_elapsed && is_tick_ready ? contract_info.contract_type : ''
      }
      digits={digits_array}
      ...
      status={
        status as React.ComponentProps<typeof LastDigitPrediction>['status']
      }
...
const Digits = React.memo((props: TDigits) => {
  ...
  const getPopoverMessage = () => {
    const underlying_name = is_trade_page
      ? underlying
    ...
    return (
      <Localize
        ...
        values={{
          underlying_name:
            getMarketNamesMap()[
              underlying_name?.toUpperCase() as keyof ReturnType<
                typeof getMarketNamesMap
              >
            ],
        }}
      />
