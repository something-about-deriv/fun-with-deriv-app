...
const LastDigitPrediction = ({
  ...
  const [digit_offset] = React.useState<
    Record<number, Record<'left' | 'top', number>>
  >({
    0: { left: 6, top: 0 },
  ...
  const getBarrier = (num: number | null): number | null => {
    const barrier_map: {
      ...
      DIGITDIFF: (val: number | null) =>
        val !== barrier && !!barrier && !isNaN(barrier),
  ...
  const isSelectableDigitType = () =>
    isMobile() ? trade_type !== 'even_odd' : false;
  ...
  const latest_tick_ask_price =
    tick?.ask && latest_tick_pip_size
      ? tick.ask.toFixed(latest_tick_pip_size)
      : null;
  const latest_tick_digit = latest_tick_ask_price
    ? +(latest_tick_ask_price.split('').pop() || '')
    : null;

  const position = tick
    ? getOffset()[latest_tick_digit ?? -1]
    : // @ts-expect-error
