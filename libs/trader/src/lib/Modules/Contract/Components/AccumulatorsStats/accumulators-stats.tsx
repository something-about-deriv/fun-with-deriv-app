...
const AccumulatorsStats = observer(
  ({ is_expandable = true }: TAccumulatorStats) => {
    const { ui } = useStore();
    ...
    const rows = ticks_history.reduce((acc: number[][], _el, index) => {
      const desktop_row_size = is_collapsed
        ? ROW_SIZES.DESKTOP_COLLAPSED
        : ROW_SIZES.DESKTOP_EXPANDED;
      const mobile_row_size = is_collapsed
        ? ROW_SIZES.MOBILE_COLLAPSED
        : ROW_SIZES.MOBILE_EXPANDED;
    ...
    return (
      <div className="accumulators-stats">
        <div className={classNames('accumulators-stats__container--collapsed')}>
          <div className="accumulators-stats__title">
            ...
            <Text
              ...
              className="accumulators-stats__title-text"
            >
              {widget_title}
            </Text>
          </div>
          <Text
            ...
            className="accumulators-stats__history"
          >
            {!is_collapsed ? (
              <div className="accumulators-stats__history-heading">
        ...
        {is_expandable && !is_collapsed && (
          <React.Fragment>
            <DesktopWrapper>
              <ExpandedTicksHistory
                history_text_size={history_text_size}
                rows={rows}
              />
            </DesktopWrapper>
            <MobileWrapper>
