...
import { displayMoney } from '@deriv/api';
import { THooks, TWalletLandingCompanyName } from '../../../../../types';
import {
  getAccountName,
  getLandingCompanyNameOfMT5Account,
  getMarketType,
} from '../../../helpers';
...
const useExtendedTransferAccountProperties = (
  ...
  const { data: activeWallet, isLoading: isActiveWalletLoading } =
    useActiveWalletAccount();
  const { getConfig, isLoading: isCurrencyConfigLoading } = useCurrencyConfig();

  const isLoading =
    isAuthorizeLoading || isCurrencyConfigLoading || isActiveWalletLoading;

  const extendedTransferAccounts = useMemo(() => {
    //populating transfer accounts with extra fields
    const updatedAccounts = accounts?.map((account) => {
      const currencyConfig = account.currency
        ? getConfig(account.currency)
        : undefined;
    ...
    const tradingAccounts =
      updatedAccounts?.filter(
        (account) => account.account_category === 'trading'
      ) || [];
