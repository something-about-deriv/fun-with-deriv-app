...
import { TSocketRequestPayload } from '@deriv/api';
...
type TFilter = NonNullable<
  TSocketRequestPayload<'statement'>['payload']
>['action_type'];
...
const TransactionsCompleted: React.FC<TProps> = ({ filter }) => {
  ...
  const { data: accounts, isLoading: isAccountsListLoading } =
    useAllAccountsList();

  const isLoading =
    isTransactionListLoading || isWalletLoading || isAccountsListLoading;
  ...
  useEffect(() => {
    if (fetchMoreOnBottomReached)
      setOnCashierScroll(() => fetchMoreOnBottomReached);
  ...
  if (!wallet || (!transactions && (isFetching || isLoading)))
    return <Loader />;
      ...
      columns={[
        {
          accessorFn: (row) =>
            ...
            moment.unix(row.transaction_time).format('DD MMM YYYY'),
          accessorKey: 'date',
          header: 'Date',
        },
      ]}
