...
const DerivAppsGetAccount: React.FC = () => {
  ...
  const { isSuccess: isAccountCreationSuccess, mutate: createNewRealAccount } =
    useCreateNewRealAccount();
  const {
    data: {
  ...
  const { data: activeLinkedToTradingAccount } =
    useActiveLinkedToTradingAccount();

  const landingCompanyName =
    activeWallet?.landing_company_name?.toLocaleUpperCase();
    ...
    show(
      ...
      >
        <CFDSuccess
          ...
          displayBalance={
            activeLinkedToTradingAccount?.display_balance ?? '0.00'
          }
