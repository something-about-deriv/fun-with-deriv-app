...
const WalletTourGuide = () => {
  const [walletsOnboarding, setWalletsOnboarding] = useLocalStorage(
    key,
    useReadLocalStorage(key)
  );
  const [addMoreWalletsTransformValue, setAddMoreWalletsTransformValue] =
    useState('');
  ...
  const addMoreWalletRef = useRef<HTMLElement | null>(
    document.getElementById('wallets_add_more_carousel_wrapper')
  );
  ...
  const hasMT5Account = Boolean(
    activeWallet?.linked_to?.some((account) => account.platform === 'mt5')
  );
  const hasDerivAppsTradingAccount = Boolean(activeWallet?.dtrade_loginid);
  const isAllWalletsAlreadyAdded = Boolean(
    availableWallets?.every((wallet) => wallet.is_added)
  );
  ...
  const callbackHandle = (data: CallBackProps) => {
    ...
    if (index === 0 && !isAllWalletsAlreadyAdded) {
      if (
        addMoreWalletRef.current &&
        lifecycle === 'init' &&
        action === 'start'
      ) {
        setAddMoreWalletsTransformValue(
          addMoreWalletRef.current.style.transform
        );
  ...
  useEffect(() => {
    if (!addMoreWalletRef.current) {
      addMoreWalletRef.current = document.getElementById(
        'wallets_add_more_carousel_wrapper'
      );
    }
  }, []);
