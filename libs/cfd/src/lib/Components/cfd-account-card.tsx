...
import {
  TMarketTypeSynthetic,
  TAccountCategory,
} from '../types/market-type.types';
...
const AddAccountButton = React.forwardRef<
  HTMLDivElement,
  { onSelectAccount: () => void; is_disabled?: boolean }
...
const CFDAccountCardAction = ({
  ...
  if (
    ...
    (platform === CFD_PLATFORMS.MT5
      ? has_real_account && type.type === 'financial_stp'
      : true)
  ) {
    return (
      <div className="cfd-account-card__action-wrapper">
        <Localize
          ...
          values={{
            platform: getCFDPlatformLabel(platform as TPlatform),
          ...
          components={[
            <a
              className={classNames(
                ...
                {
                  'cfd-account-card__action-wrapper__link--disabled':
                    is_accounts_switcher_on,
                }
              )}
              key={0}
...
const CFDAccountCardComponent = observer(
  ({
    ...
    const existing_data = existing_accounts_data?.length
    ...
    const should_show_extra_add_account_button =
      ...
      (type.category === CATEGORY.DEMO
        ? isEligibleForMoreDemoMt5Svg(
            type.type as
              | TTradingPlatformAvailableAccount['market_type']
              | TMarketTypeSynthetic
          ) && !!existing_data
        : isEligibleForMoreRealMt5(
            type.type as
    ...
    const platform_icon =
      show_eu_related_content && platform === CFD_PLATFORMS.MT5
        ? CFD_PLATFORMS.CFD
        : type.type;
    ...
    const getServerName: (value: DetailsOfEachMT5Loginid) => string =
      React.useCallback((server) => {
        if (server) {
          const server_region = (server as DetailsOfEachMT5Loginid).server_info
            ?.geolocation?.region;
          if (server_region) {
            return `${server_region} ${
              (server as DetailsOfEachMT5Loginid)?.server_info?.geolocation
                ?.sequence === 1
                ? ''
                : (server as DetailsOfEachMT5Loginid)?.server_info?.geolocation
                    ?.sequence
    ...
    const getBannerStatus = (account: DetailsOfEachMT5Loginid) => {
      ...
      if (
        ...
        status &&
        ['proof_failed', 'verification_pending'].includes(status)
      ) {
    ...
    const is_web_terminal_unsupported =
        ...
        >
          ...
          <div
            className={classNames('cfd-account-card__type', {
              'cfd-account-card__type--has-banner':
          ...
          <div
            ...
            style={!existing_data?.login ? { marginTop: 'auto' } : {}}
          >
            <div className="cfd-account-card__cta-wrapper">
              ...
              {existing_data?.login &&
                ...
                existing_accounts_data?.map(
                  (acc: FormikValues, index: number) => (
                    <div className="cfd-account-card__item" key={index}>
                      ...
                      {acc?.display_balance && is_logged_in && (
                        <div className="cfd-account-card__balance">
                          <Text
                            size="xxl"
                            className="cfd-account-card__balance--value"
                          >
                            <Money
              ...
              {existing_data?.login &&
                ...
                type.category === CATEGORY.DEMO && (
                  <div className="cfd-account-card__item">
                    {(existing_data as TTradingPlatformAccounts)
                      ?.display_login && (
