  ...
  <Field name={name}>
    {({
      ...
      <Input
        ...
        error={
          touched[field.name as keyof TFormValues] &&
          errors[field.name as keyof TFormValues]
        }
        {...props}
      />
    )}
...
const validatePersonalDetails = ({
  ...
  const tin_format = residence_list.find(
    (res) => res.text === values.tax_residence
  )?.tin_format;

  const tin_regex = tin_format || ['^[A-Za-z0-9./s-]{0,25}$']; // fallback to API's default rule check

  const validations: {
    [key: string]: ((v: string) => boolean | RegExpMatchArray | null)[];
  } = {
    citizen: [
      (v: string) => !!v,
      (v: string) => residence_list.map((i) => i.text).includes(v),
    ],
    tax_residence: [
      (v: string) => !!v,
      (v: string) => residence_list.map((i) => i.text).includes(v),
    ...
    tax_identification_number: [
      (v: string) => !!v,
      (v: string) =>
        tin_regex ? tin_regex?.some((regex) => v.match(regex)) : true,
...
const submitForm: TSubmitForm = (
  ...
  const { citizen, place_of_birth, tax_residence } =
    findDefaultValuesInResidenceList({
      residence_list,
...
const CFDPersonalDetailsForm = ({
  ...
  const isFieldDisabled = (field: string) =>
    !!initial_values[field] && !changeable_fields?.includes(field);

  if (is_loading || residence_list.length === 0)
    ...
    >
      {({
        ...
        return (
          <AutoHeightWrapper
            default_height={200}
            height_offset={isDesktop() ? 148 : null}
          >
            {({
              setRef,
            }: {
              ...
              >
                        ...
                        <DesktopWrapper>
                          <Field name="citizen">
                            {({ field }: FieldProps<string, TFormValues>) => (
                              // @ts-expect-error: TODO: Fix typings for Autocomplete
                              <Autocomplete
                                {...field}
                ...
                <Modal.Footer is_bypassed={isMobile()} has_separator>
                  {form_error && (
                    <FormSubmitErrorMessage message={form_error} />
                  )}
                  <FormSubmitButton
                    is_disabled={
