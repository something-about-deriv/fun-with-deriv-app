...
const mock_errors = {
  ...
  repeated_chars_are_easy: () =>
    localize('Repeats like "aaa" are easy to guess'),
...
describe('<CFDPasswordManagerModal />', () => {
  ...
  const renderwithRouter = (component) => {
    render(<BrowserRouter>{component}</BrowserRouter>, {
      wrapper: ({ children }) => (
        <CFDProviders store={mockStore(mockRootStore)}>{children}</CFDProviders>
      ),
    });
  };
  let modal_root_el;
  ...
  it('should render CFDPasswordManagerModal if change password button is clicked', () => {
    ...
    expect(
      screen.getByText(/confirm to change your deriv mt5 password/i)
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /this will change the password to all of your deriv mt5 accounts/i
      )
    ).toBeInTheDocument();
  ...
  it('should render SentEmailModal if confirm button is clicked', async () => {
    ...
    expect(
      await screen.findByText(/we've sent you an email/i)
    ).toBeInTheDocument();
    expect(
    ...
    expect(
      await screen.findByText(/didn't receive the email?/i)
    ).toBeInTheDocument();
  });

  it('should render the CFDPasswordManagerModal for deriv x', () => {
    renderwithRouter(
      <CFDPasswordManagerModal {...mock_props} platform="dxtrade" />
    );
    ...
    expect(
      screen.getByText(
        /use this password to log in to your deriv x accounts on the web and mobile apps/i
      )
    ).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: /change password/i })
  ...
  it('should render ChangePasswordConfirmationModal if change deriv x button is clicked', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /change password/i }));
    expect(
      screen.getByText(/confirm to change your deriv x password/i)
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /this will change the password to all of your deriv x accounts/i
      )
    ).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /cancel/i })).toBeInTheDocument();
    expect(
  ...
  it("should render the resend email button if a user clicks didn't receive the email", async () => {
    ...
    expect(
      await screen.findByRole('button', { name: /resend email/i })
    ).toBeInTheDocument();
  });
  ...
  it('should disable the button for 60 seconds after clicking resend email', async () => {
    ...
    expect(
      screen.getByRole('button', { name: /resend email in 59/i })
    ).toBeDisabled();
  });
  ...
  it('should change input of current investor password and new investor password and trigger change investor password button', async () => {
    ...
    const current_investor_password = screen.getByLabelText(
      /current investor password/i
    );
    const new_investor_password = screen.getByLabelText(
      'New investor password'
    );

    fireEvent.change(current_investor_password, {
      target: { value: 'Testing1234' },
    });
    fireEvent.blur(current_investor_password);
    fireEvent.change(new_investor_password, {
      target: { value: 'XCvnhnkdh!111' },
    });
    ...
    expect(
      screen.getByRole('button', { name: /change investor password/i })
    ).toBeEnabled();
  ...
  it('should render the correct header title for investor password modal for real account', async () => {
    renderwithRouter(
      <CFDPasswordManagerModal {...mock_props} selected_account_group="real" />
    );
    expect(screen.getByText(/investor password/i)).toBeInTheDocument();
    fireEvent.click(screen.getByText(/investor password/i));

    expect(
      await screen.findByText(/manage deriv mt5 password/i)
    ).toBeInTheDocument();
  });
});
