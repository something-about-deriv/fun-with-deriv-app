...
describe('<InvestorPasswordManager> ', () => {
  ...
  it('should render the correct texts ', async () => {
    ...
    expect(
      await screen.findByText(/new investor password/i)
    ).toBeInTheDocument();
    expect(
      screen.getByText(
  ...
  it('should fill the password field and trigger the appropriate error message for repeated password pattern', async () => {
    ...
    expect(
      await screen.findByText(
        /repeats like "abcabcabc" are only slightly harder to guess than "abc"/i
      )
    ).toBeInTheDocument();
  });
  ...
  it('should fill the password field and trigger the appropriate error message for repeated characters', async () => {
    ...
    expect(
      await screen.findByText(/repeats like "aaa" are easy to guess/i)
    ).toBeInTheDocument();
  });
  ...
  it('should fill the password field and trigger the appropriate error message for using recent years', async () => {
    ...
    expect(
      await screen.findByText(/recent years are easy to guess/i)
    ).toBeInTheDocument();
  });
  ...
  it('should fill the password fields and trigger the appropriate message and enable the change password button', async () => {
    ...
    const current_investor = screen.getByLabelText(
      /current investor password/i
    );
    const new_investor = screen.getByLabelText(/new investor password/i);
    const change_investor_password_btn = screen.getByText(
      /change investor password/i
    );
    await waitFor(() => {
      ...
      expect(
        screen.getByTestId('dt_error_message_investor')
      ).toBeInTheDocument();
  ...
  it('should render success message if the user clicks on create or reset investor passwords', async () => {
    render(
      <InvestorPasswordManager
        {...mock_props}
        is_submit_success_investor={true}
      />
    );
    expect(
      screen.getByText(/your investor password has been changed/i)
    ).toBeInTheDocument();
