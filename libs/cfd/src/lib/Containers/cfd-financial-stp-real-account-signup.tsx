...
const CFDFinancialStpRealAccountSignup = observer(
  ({ onFinish }: TCFDFinancialStpRealAccountSignupProps) => {
    ...
    const { need_poi_for_maltainvest, need_poi_for_bvi_labuan_vanuatu } =
      getAuthenticationStatusInfo(account_status);

    const is_authenticated_with_idv_photoid = useIsAccountStatusPresent(
    ...
    const shouldShowPOA = () => {
      ...
      return !['pending', 'verified'].includes(
    ...
    const [items, setItems] =
      React.useState<TItemsState<typeof passthroughProps>[]>(
    ...
    const passthrough: Partial<TCFDFinancialStpRealAccountSignupProps> & {
      ...
      (getCurrent('forwarded_props') || []) as TItemsState<
        typeof passthroughProps
      >['forwarded_props']
    ...
    if (shouldShowPOA()) {
      passthrough.is_authenticated_with_idv_photoid =
        is_authenticated_with_idv_photoid;
    }
    ...
    return (
      ...
      >
        <div
          ...
          data-testid="dt_cfd_financial_stp_modal_body"
        >
          <BodyComponent
