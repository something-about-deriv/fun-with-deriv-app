  ...
  const config = {
    ...
    first_name: {
      ...
      rules: [
        ...
        [
          'length',
          localize('First name should be between 2 and 50 characters.'),
          { min: 2, max: 50 },
        ],
    ...
    last_name: {
      ...
      rules: [
        ...
        [
          'length',
          localize('Last name should be between 2 and 50 characters.'),
          { min: 2, max: 50 },
        ],
        ['name', getErrorMessages().name()],
      ],
    ...
    date_of_birth: {
      ...
      rules: [
        ...
        [
          (v: string) =>
            toMoment(v).isValid() &&
            toMoment(v).isBefore(toMoment().subtract(18, 'years')),
    ...
    place_of_birth: {
      ...
      default_value:
        (account_settings.place_of_birth &&
          residence_list.find(
            (item) => item.value === account_settings.place_of_birth
          )?.text) ||
        '',
    ...
    citizen: {
      ...
      default_value:
        (account_settings.citizen &&
          residence_list.find((item) => item.value === account_settings.citizen)
            ?.text) ||
        '',
    ...
    tax_residence: {
      ...
      default_value:
        (account_settings?.tax_residence &&
          residence_list.find(
            (item) => item.value === account_settings?.tax_residence
          )?.text) ||
        default_residence,
    ...
    tax_identification_number: {
      ...
      rules: [
        ...
        [
          ...
          (value: string) =>
            value
              ? RegExp(/^(?!^$|\s+)[A-Za-z0-9./\s-]{0,25}$/).test(value)
              : true,
          localize(
            'Letters, numbers, spaces, periods, hyphens and forward slashes only.'
          ),
        ],
        [
          (
        ...
        [
          (
            ...
            return value && tin_format
              ? tin_format.some((regex) => new RegExp(regex).test(value))
              : true;
          },
  ...
  if (
    ...
    properties_to_update.forEach((key) => {
      ...
      if (key === 'tax_identification_number' || key === 'tax_residence') {
        config[key].rules = config[key].rules.filter(
          (rule) => rule[0] !== 'req'
        );
      }
    });
...
const transformConfig = (
  ...
  if (
    ['malta', 'iom'].includes(real_account_signup_target) &&
    config.tax_residence
  ) {
