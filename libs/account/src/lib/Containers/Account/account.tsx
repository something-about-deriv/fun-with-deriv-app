...
const Account = observer(({ history, location, routes }: TAccountProps) => {
  ...
  const selected_content = subroutes.find((r) =>
    matchRoute(r, location.pathname)
  );
    ...
    if (menu_item?.subroutes?.length) {
      menu_item.subroutes.forEach((route) => {
        ...
        if (route.path === shared_routes.trading_assessment) {
          route.is_disabled =
            is_virtual || landing_company_shortcode !== 'maltainvest';
        }

        if (
          route.path === shared_routes.proof_of_identity ||
          route.path === shared_routes.proof_of_address
        ) {
