 ...
 */
const PageOverlayWrapper = observer(
  ({ routes, subroutes }: PageOverlayWrapperProps) => {
    ...
    const list_groups = routes.map((route_group) => ({
      ...
      subitems: route_group?.subroutes?.length
        ? // @ts-expect-error fix this
          route_group.subroutes.map((sub) => subroutes.indexOf(sub))
        : [],
    }));
    ...
    const selected_route = getSelectedRoute({
      // @ts-expect-error fix this
      routes: subroutes as Array<TRoute>,
    ...
    if (is_mobile && selected_route) {
      const RouteComponent = selected_route.component as React.ElementType<{
        component_icon: string | undefined;
      }>;
      return (
        <PageOverlay
