...
jest.mock('@deriv/shared', () => ({
  ...
  isDesktop: jest.fn(),
  isMobile: jest.fn(),
  makeCancellablePromise: jest.fn(() => ({
    cancel: jest.fn(),
...
describe('<AddressDetails/>', () => {
  ...
  const use_address_info =
    /only use an address for which you have proof of residence/i;
  ...
  const svgCommonRenderCheck = () => {
    ...
    expect(
      screen.queryByLabelText(address_line_2_marked)
    ).not.toBeInTheDocument();
    expect(
      screen.queryByLabelText(address_postcode_marked)
    ).not.toBeInTheDocument();
  ...
  it('should render AddressDetails component and trigger buttons', async () => {
    ...
    const address_line_1_input: HTMLInputElement = screen.getByLabelText(
      address_line_1_marked
    );
    const first_line_adress_text = 'Test first line address';
    expect(address_line_1_input.value).toBe('');
    fireEvent.change(address_line_1_input, {
      target: { value: first_line_adress_text },
    });
    await waitFor(() => {
      expect(address_line_1_input.value).toBe(first_line_adress_text);
    });

    const address_town_input: HTMLInputElement =
      screen.getByLabelText(address_town_marked);
    const address_town_text = 'Test city';
    expect(address_town_input.value).toBe('');
    fireEvent.change(address_town_input, {
      target: { value: address_town_text },
    });
    await waitFor(() => {
      expect(address_town_input.value).toBe(address_town_text);
    });

    const address_postcode_input: HTMLInputElement =
      screen.getByLabelText(address_postcode);
    const address_postcode_text = 'Test postcode';
    expect(address_postcode_input.value).toBe('');
    fireEvent.change(address_postcode_input, {
      target: { value: address_postcode_text },
    });
    await waitFor(() => {
      expect(address_postcode_input.value).toBe(address_postcode_text);
    });

    const address_state_input: HTMLInputElement =
      screen.getByLabelText(address_state);
    const address_state_text = 'Test state';
    expect(address_state_input.value).toBe('Default test state');
    fireEvent.change(address_state_input, {
      target: { value: address_state_text },
    });
    await waitFor(() => {
  ...
  it('should render AddressDetails component with states_list for desktop', async () => {
    ...
    const address_state_input: HTMLTextAreaElement = screen.getByRole(
      'textbox',
      { name: 'State/Province' }
    );
