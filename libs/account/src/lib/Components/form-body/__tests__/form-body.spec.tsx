...
describe('<FormBody />', () => {
  it('should render FormBody component with children in desktop', () => {
    ...
    expect(div_wrapper_sc).toHaveClass(
  ...
  it('should render FormBody component with children in mobile', () => {
    ...
    expect(
      screen.queryByTestId('dt_scrollbar_container_div')
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId('dt_themed_scrollbars')
    ).not.toBeInTheDocument();
    const div_100_vh = screen.getByTestId('dt_div_100_vh');
    expect(div_100_vh).toBeInTheDocument();
    expect(div_100_vh).toHaveClass(
      'account__scrollbars_container--grid-layout'
    );
