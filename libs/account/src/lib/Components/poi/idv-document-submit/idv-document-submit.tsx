...
type TIdvDocumentSubmitForm = TIDVFormValues &
  TPersonalDetailsForm & { confirmation_checkbox: boolean };
...
const IdvDocumentSubmit = observer(
  ({
    ...
    if (form_initial_values.date_of_birth) {
      form_initial_values.date_of_birth = toMoment(
        form_initial_values.date_of_birth
      ).format('YYYY-MM-DD');
    }
    ...
    const submitHandler = async (
      values: TIdvDocumentSubmitForm,
      {
        ...
        setStatus,
        status,
      }: FormikHelpers<TIdvDocumentSubmitForm> &
        FormikState<TIdvDocumentSubmitForm>
    ) => {
      setSubmitting(true);
      ...
      if (get_settings?.error) {
        setStatus({
          ...status,
          error_message: get_settings?.error?.message ?? GENERIC_ERROR_MESSAGE,
        });
        setSubmitting(false);
        return;
      }
      ...
      >
        {({ dirty, isSubmitting, isValid, values, status }) => (
          <Form className="proof-of-identity__container proof-of-identity__container--reset">
            ...
            <FormBody
              className="form-body"
              scroll_offset={isMobile() ? '180px' : '80px'}
            >
              ...
              <PersonalDetailsForm
                ...
                is_rendered_for_idv
                editable_fields={
                  values.confirmation_checkbox ? [] : changeable_fields
                }
