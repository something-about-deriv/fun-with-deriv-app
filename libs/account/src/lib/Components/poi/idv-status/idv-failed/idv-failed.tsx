...
const IdvFailed = ({
  ...
  const chosen_country = React.useMemo(
    () =>
      is_document_upload_required && !is_from_external
        ...
        : residence_list.find(
            (residence_data) =>
              residence_data.value === latest_status?.country_code
          ) ?? {},
    [
      selected_country,
      is_document_upload_required,
      latest_status?.country_code,
      residence_list,
      is_from_external,
    ]
  );
  ...
  React.useEffect(() => {
    const initializeFormValues = async (required_fields: string[]) => {
      ...
      if (form_data.date_of_birth) {
        form_data.date_of_birth = toMoment(form_data.date_of_birth).format(
          'YYYY-MM-DD'
        );
      }
  ...
  const onSubmit = async (
    ...
    FormikHelpers<TIdvFailedForm> & FormikState<TIdvDocumentSubmitForm>
  ) => {
    ...
    if (data.error) {
      ...
      if (
        !submit_data.document_type ||
        submit_data.document_type === IDV_NOT_APPLICABLE_OPTION.id
      ) {
        setSubmitting(false);
        handleSubmit();
        return;
    ...
    if (is_document_upload_required) {
      ...
      if (!shouldHideHelperImage(document_type?.id as string)) {
        if (needs_additional_document) {
          errors.document_additional = isAdditionalDocumentValid(
            document_type,
            document_additional
          );
        }
        errors.document_number = isDocumentNumberValid(
          document_number ?? '',
          document_type as FormikValues
        );
      }
    }
    // @ts-expect-error fix this
  ...
  if (rest_state?.api_error)
    return <LoadErrorMessage error_message={rest_state.api_error} />;

  if (
    is_loading &&
    Object.keys(rest_state?.form_initial_values ?? {}).length === 0
  ) {
    return (
