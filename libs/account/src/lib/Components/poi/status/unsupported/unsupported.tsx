...
const Unsupported = ({
  ...
  if (manual) {
    if (manual.status === identity_status_codes.pending)
      ...
      ].includes(manual.status as 'rejected' | 'suspected')
    ) {
      if (!allow_poi_resubmission) return <Limited />;
    } else if (manual.status === identity_status_codes.verified) {
      return (
        <Verified needs_poa={needs_poa} redirect_button={redirect_button} />
      );
  ...
  return (
    ...
    >
      <Timeline.Item
        item_title={localize('Please upload one of the following documents:')}
      >
