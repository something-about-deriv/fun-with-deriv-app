...
const DetailComponent = ({
  ...
  const uploadFiles = (data: FormikValues) =>
    new Promise((resolve, reject) => {
      const docs = document.details.documents.map(
        (item: FormikValues) => item.name
      );
      const files = Object.values(data).filter((item) =>
        [...docs, SELFIE_DOCUMENT.name].includes(item.name)
      );
          ...
          .then((response) => {
            ...
            if (response.warning || response.error) {
              ...
              setError(
                ...
                response.message ||
                  // @ts-expect-error fix this
                  (response.error ? response.error.message : localize('Failed'))
              );
    ...
    case STATUS.IS_UPLOADING:
      return (
        <Loading
          is_fullscreen={false}
          is_slow_loading
          status={[localize('Uploading documents')]}
        />
      );
      ...
      return (
        <React.Fragment>
          {is_onfido_supported ? (
            <React.Fragment>
              <div
                className={`${root_class}__detail-header`}
                onClick={onClickBack}
              >
              ...
              <OnfidoUpload
                getChangeableFields={null}
                country_code={country_code_key}
                documents_supported={[document.onfido_name]}
                height={height ?? null}
                handleComplete={
                  is_mt5 ? handlePOIforMT5Complete : handleComplete
                }
                is_default_enabled
                handleViewComplete={
                  is_mt5 ? handlePOIforMT5Complete : handleComplete
