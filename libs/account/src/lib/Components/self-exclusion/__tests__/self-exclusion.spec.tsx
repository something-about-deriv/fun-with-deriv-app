...
jest.mock('@deriv/sharedservices/ws-methods', () => ({
...
describe('<SelfExclusion />', () => {
  ...
  it('should render SelfExclusion component for virtual account', () => {
    ...
    expect(
      screen.getByText('This feature is not available for demo accounts.')
    ).toBeInTheDocument();
  });
  ...
  it('should render SelfExclusion component with error', async () => {
    ...
    expect(
      screen.queryByText('Test getSelfExclusion response error')
    ).toBeInTheDocument();
  });
  ...
  it('Should trigger session_duration_limit input and show error if the value is greater than 60480 or does not show if less than 60480', async () => {
    ...
    await act(async () => {
      if (session_duration_limit_input)
        fireEvent.change(session_duration_limit_input, {
          target: { value: '60481' },
        });
    });
  ...
  it('Should trigger exclude_until input and show error depends on input value', async () => {
    ...
    const inputs = await screen.findAllByRole('textbox');
    const exclude_until_input = inputs.find(
      (input: FormikValues) => input.name === 'exclude_until'
    );

    await act(async () => {
      if (exclude_until_input)
        fireEvent.change(exclude_until_input, {
          target: { value: '2021-04-13' },
        });
    });

    expect(
      screen.getByText('Exclude time must be after today.')
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText('Exclude time cannot be less than 6 months.')
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText('Exclude time cannot be for more than five years.')
    ).toBeInTheDocument();
  });
  ...
  it('should trigger inputs with data, add new data, and show error wih invalid input data', async () => {
    ...
    const max_turnover_input = inputs_1.find(
      (input: FormikValues) => input.name === 'max_turnover'
    );
    const max_open_bets_input = inputs_1.find(
      (input: FormikValues) => input.name === 'max_open_bets'
    );

    act(() => {
      if (max_turnover_input)
        fireEvent.change(max_turnover_input, { target: { value: '1700' } });
      if (max_open_bets_input)
    ...
    expect(
      screen.getByText('You have set the following limits:')
    ).toBeInTheDocument();
    ...
    expect(
      screen.queryByText('Your stake and loss limits')
    ).not.toBeInTheDocument();
    ...
    expect(screen.getByText('Your stake and loss limits')).toBeInTheDocument();
    expect(
      screen.queryByText('You have set the following limits:')
    ).not.toBeInTheDocument();
    ...
    const max_balance_input = inputs_1.find(
      (input: FormikValues) => input.name === 'max_balance'
    );

    act(() => {
      if (max_balance_input)
    ...
    expect(
      screen.getByText('Test setSelfExclusion response error')
    ).toBeInTheDocument();
  });
  ...
  it('should trigger inputs with correct data set timeout limit and logout', async () => {
    ...
    const max_open_bets_input = inputs.find(
      (input: FormikValues) => input.name === 'max_open_bets'
    );
    const max_losses_input = inputs.find(
      (input: FormikValues) => input.name === 'max_losses'
    );
    const max_turnover_input = inputs.find(
      (input: FormikValues) => input.name === 'max_turnover'
    );
    const max_7day_losses_input = inputs.find(
      (input: FormikValues) => input.name === 'max_7day_losses'
    );
    const max_7day_turnover_input = inputs.find(
      (input: FormikValues) => input.name === 'max_7day_turnover'
    );
    const max_30day_losses_input = inputs.find(
      (input: FormikValues) => input.name === 'max_30day_losses'
    );
    const max_30day_turnover_input = inputs.find(
      (input: FormikValues) => input.name === 'max_30day_turnover'
    );
    const session_duration_limit_input = inputs.find(
      (input: FormikValues) => input.name === 'session_duration_limit'
    );
    ...
    expect(
      screen.queryByText('Yes, log me out immediately')
    ).not.toBeInTheDocument();
