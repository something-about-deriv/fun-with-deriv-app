...
const IDVForm = ({
  ...
  const { documents_supported: document_data } =
    selected_country?.identity?.services?.idv ?? {};
  ...
  React.useEffect(() => {
    if (document_data && selected_country && selected_country.value) {
      ...
      const new_document_list = filtered_documents.map((key) => {
        ...
        const {
          new_display_name,
          example_format,
          additional_document_example_format,
    ...
    document_data,
    selected_country,
    can_skip_document_verification,
              ...
              <div className="proof-of-identity__fieldset-container">
                <fieldset
                  className={classNames({
                    'proof-of-identity__fieldset': !hide_hint,
                  })}
                >
