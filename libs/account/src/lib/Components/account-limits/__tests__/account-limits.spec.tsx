...
jest.mock('Components/load-error-message', () =>
  jest.fn(() => 'mockedLoadErrorMessage')
);
jest.mock('../account-limits-footer', () =>
  jest.fn(() => 'mockedAccountLimitsFooter')
);
...
describe('<AccountLimits/>', () => {
  ...
  it('should render DemoMessage component if is_virtual is true', () => {
    ...
    expect(screen.queryByTestId('dt_account_demo_message_wrapper')).toHaveClass(
  ...
  it('should render AccountLimitsArticle component if should_show_article is true in mobile mode', () => {
    ...
    expect(
      screen.getByRole('heading', { name: /account limits/i })
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /these are default limits that we apply to your accounts\./i
      )
    ).toBeInTheDocument();
  });
  ...
  it('should render Trading limits table and its maximum daily turnover contents properly', () => {
    ...
    expect(
      screen.queryByTestId('trading_daily_turnover_table')
    ).toBeInTheDocument();
    expect(
      screen.getByRole('columnheader', {
  ...
  it('should not render withdrawal_limits_table is_app_settings is true', () => {
    ...
    expect(
      screen.queryByTestId('withdrawal_limits_table')
  ...
  it('withdrawal_limits_table should have a Limits header if is_fully_authenticated is true', () => {
    ...
    expect(screen.getByTestId('withdrawal_limits_table')).toHaveTextContent(
      'Limit'
    );
  });
  ...
  it('show show withdrawal limit lifted message if is_fully_authenticated is true', () => {
    ...
    expect(
      screen.getByText(
        /your account is fully authenticated and your withdrawal limits have been lifted\./i
      )
    ).toBeInTheDocument();
  });
  ...
  it('withdrawal_limits_table should show the verfiy button when is_fully_authenticated is false and is_appstore is true', () => {
    ...
    expect(
      screen.getByText(/to increase limit please verify your identity/i)
    ).toBeInTheDocument();
    expect(
  ...
  it('withdrawal_limits_table should show total withdrawn and withdrawn remaining details', () => {
    ...
    const { withdrawal_since_inception_monetary, remainder } =
    ...
    expect(
      screen.getByText(/maximum withdrawal remaining/i)
    ).toBeInTheDocument();
    expect(formatMoney).toHaveBeenCalledWith(
      store.client.currency,
      remainder,
  ...
  it('should show limit_notice message when is_appstore is false and is_fully_authenticated is false in mobile mode', () => {
    ...
    expect(
      screen.getByText(
        /stated limits are subject to change without prior notice\./i
      )
    ).toBeInTheDocument();
  });
