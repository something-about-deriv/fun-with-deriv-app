...
import AccountLimitsContext, {
  TAccountLimitsContext,
} from './account-limits-context';
  ...
  ({
    ...
    React.useEffect(() => {
      if (
        ...
        account_limits &&
        is_loading &&
        Object.keys(account_status).length > 0
      ) {
        setLoading(false);
      }
    ...
    const { commodities, forex, indices, synthetic_index } = {
      ...market_specific,
    };
    const forex_ordered = forex
      ?.slice()
      .sort((a: FormikValues, b: FormikValues) => a.name.localeCompare(b.name));
        ...
        >
          ...
          >
            ...
            <div className="da-account-limits__table-wrapper">
              <ThemedScrollbars
                is_bypassed={!!should_bypass_scrollbars || isMobile()}
              >
                <table
