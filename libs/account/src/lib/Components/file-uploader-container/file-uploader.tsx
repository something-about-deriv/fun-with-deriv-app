...
const FileUploader = ({ onFileDrop, onError }: TFileUploaderProps) => {
  ...
  const handleRejectedFiles: THandleRejectedFiles = (files) => {
    const is_file_too_large =
      files.length > 0 && files[0].file.size > max_document_size;
    const supported_files = files.filter((each_file) =>
      getSupportedFiles(each_file.file.name)
    );
    const error_message =
