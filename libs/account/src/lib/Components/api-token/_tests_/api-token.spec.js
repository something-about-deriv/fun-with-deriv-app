...
describe('<ApiToken/>', () => {
  ...
  const token_creation_description =
  ...
  const trading_info_description =
  ...
  it('should render ApiToken component without app_settings and footer', async () => {
    ...
    expect(
      await screen.findByText(admin_scope_description)
    ).toBeInTheDocument();
    expect(await screen.findByText(admin_scope_note)).toBeInTheDocument();
    expect(
      await screen.findByText(trading_info_scope_description)
    ).toBeInTheDocument();
    expect(await screen.findByText(select_scopes_msg)).toBeInTheDocument();
    expect(
      await screen.findByText(token_creation_description)
    ).toBeInTheDocument();
    expect(
      await screen.findByText(token_using_description)
    ).toBeInTheDocument();
    expect(
      await screen.findByText(trade_scope_description)
    ).toBeInTheDocument();
    expect(
      await screen.findByText(trading_info_description)
    ).toBeInTheDocument();
    expect(
      await screen.findByText(your_access_description)
    ).toBeInTheDocument();
  ...
  it('should not render ApiToken component if is not mounted', () => {
    ...
    expect(screen.queryByText(learn_more_title)).not.toBeInTheDocument();
    expect(
      screen.queryByText(trading_info_scope_description)
    ).not.toBeInTheDocument();
    expect(screen.queryByText(select_scopes_msg)).not.toBeInTheDocument();
    expect(
      screen.queryByText(token_creation_description)
    ).not.toBeInTheDocument();
    ...
    expect(
      screen.queryByText(trading_info_description)
    ).not.toBeInTheDocument();
  ...
  it('should render ApiToken component with app_settings', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByText(our_access_description)
      ).not.toBeInTheDocument();
    });
  });
  ...
  it('should choose checkbox, enter a valid value and create token', async () => {
    ...
    expect(
      await screen.findByText(/length of token name must be between/i)
    ).toBeInTheDocument();

    fireEvent.change(token_name_input, {
      target: { value: 'New test extra long name for erorr' },
    });
  ...
  it('should trigger hide/unhide icon and trigger copy icon, should show dialog only for admin scope', async () => {
    ...
    const toggle_visibility_btns = await screen.findAllByTestId(
    ...
    expect(
      screen.queryByTestId('dt_token_copied_icon')
    ).not.toBeInTheDocument();
