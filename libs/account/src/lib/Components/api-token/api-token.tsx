...
const ApiToken = ({
  ...
  const toggleOverlay = () =>
    setState({ is_overlay_shown: !state.is_overlay_shown });
  ...
  const validateFields = (values: FormikValues) => {
    ...
    if (!token_name) {
      ...
      errors.token_name = localize('Maximum {{MAX_TOKEN}} characters.', {
        MAX_TOKEN,
      });
    }
  ...
  const populateTokenResponse = (
    response: import('@deriv/api-types').APITokenResponse
  ) => {
    if (!isMounted()) return;
    if (response.error) {
      setState({
  ...
  const deleteToken = async (token: string) => {
    ...
    const token_response = await WS.authorized.apiToken({
      api_token: 1,
      delete_token: token,
    });
