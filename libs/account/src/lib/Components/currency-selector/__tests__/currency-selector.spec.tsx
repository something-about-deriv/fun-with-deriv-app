...
describe('<CurrencySelector/>', () => {
  ...
  const runCommonTests = (msg: string) => {
    expect(
      screen.getByRole('heading', { name: /fiat currencies/i })
    ).toBeInTheDocument();
    expect(
      screen.getByRole('radio', { name: /us dollar \(usd\)/i })
    ).toBeInTheDocument();
    expect(
      screen.getByRole('radio', { name: /euro \(eur\)/i })
    ).toBeInTheDocument();

    expect(
      screen.getByRole('heading', { name: /cryptocurrencies/i })
    ).toBeInTheDocument();
    expect(
      screen.getByRole('radio', { name: /tether erc20 \(eusdt\)/i })
    ).toBeInTheDocument();
    expect(
      screen.getByRole('radio', { name: /usd coin \(usdc\)/i })
    ).toBeInTheDocument();

    expect(screen.getByRole('button', { name: /next/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /next/i })).toBeDisabled();

    const usd = screen.getByRole('radio', {
      name: /us dollar \(usd\)/i,
    }) as HTMLInputElement;
  ...
  it('should render Cryptocurrencies and submit the form ', async () => {
    ...
    const set_currency_btn = screen.getByRole('button', {
      name: /set currency/i,
    });
  ...
  it('should render the selector__container with proper div height', () => {
    ...
    expect(
      screen.getByTestId('currency_selector_form').childNodes[0]
    ).toHaveStyle('height: calc(150px - 89px);');
  });
  ...
  it('should call handleCancel when previous button is called', () => {
    ...
    const usdc: HTMLInputElement = screen.getByRole('radio', {
      name: /usd coin \(usdc\)/i,
    });
