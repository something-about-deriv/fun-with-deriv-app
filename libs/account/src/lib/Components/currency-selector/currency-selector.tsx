...
type TCurrencySelector = React.HTMLAttributes<
  HTMLInputElement | HTMLLabelElement
> &
  TCurrencySelectorExtend;
...
const CurrencySelector = observer(
  ({
    ...
    const fiat = legal_allowed_currencies.filter(
      (selected_currency) =>
        String(selected_currency.type) === CURRENCY_TYPE.FIAT
    );
    ...
    React.useEffect(() => {
      if (is_bypass_step && real_account_signup?.error_details) {
        ...
        const route_to_address_details = Object.keys(
          getAddressDetailsFields()
    ...
    return (
      ...
      >
        {({
          handleSubmit,
          values,
        }: FormikState<TCurrencySelectorFormProps> & FormikHandlers) => (
