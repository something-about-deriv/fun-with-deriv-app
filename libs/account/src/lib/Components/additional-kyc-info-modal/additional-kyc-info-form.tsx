...
export const AdditionalKycInfoForm = observer(
  ({ setError }: TAdditionalKycInfoFormProps) => {
    ...
    const onSubmit = (values: typeof initialValues) => {
      const place_of_birth = residence_list?.find(
        (item) => item.text === values.place_of_birth
      )?.value;
      ...
      if (values.tax_residence) {
        const tax_residence = residence_list?.find(
      ...
      >
        {({ isValid, setFieldValue }) => (
          <Form className="additional-kyc-info-modal__form-layout">
            ...
            <Modal.Footer
              has_separator
              className="additional-kyc-info-modal__form-action"
            >
              <Button
                large
                primary
