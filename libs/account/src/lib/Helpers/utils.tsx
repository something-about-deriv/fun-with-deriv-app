...
export const documentAdditionalError = (
  ...
  if (!additional_document_value) {
    error_message = localize('Please enter your {{document_name}}. ', {
      document_name:
        document_additional_config?.display_name?.toLowerCase() ??
...
export const preventEmptyClipboardPaste = (
...
export const isFieldImmutable = (
  field: string,
  mutable_fields: string[] = []
) => !mutable_fields.includes(field);
...
export const isAdditionalDocumentValid = (
  document_type: FormikValues,
  additional_document_value?: string
) => {
  const error_message = documentAdditionalError(
    additional_document_value,
    document_type?.additional
  );
  if (error_message) {
    return (
      localize(error_message) +
...
export const isDocumentNumberValid = (
...
export const shouldHideHelperImage = (document_id: string) =>
  document_id === IDV_NOT_APPLICABLE_OPTION.id;
...
export const convertAlpha2toAlpha3 = (country_code: string) =>
  country_code.length !== 3
    ? countries.alpha2ToAlpha3(country_code.toUpperCase())
    : country_code;
...
export const convertAlpha3toAlpha2 = (country_code: string) =>
  country_code.length !== 2
    ? countries.alpha3ToAlpha2(country_code.toUpperCase())
    : country_code;
...
export const getIDVDocumentType = (
  ...
  if (!idv_latest_attempt || !Object.keys(residence).length)
    return localize('identity document');
...
type TIDVErrorStatus =
  (typeof idv_error_statuses)[keyof typeof idv_error_statuses];
