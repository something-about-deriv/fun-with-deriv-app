...
const getCurrDxtradeAccount = (dxtrade_accounts_list, login_id) =>
  dxtrade_accounts_list.find(
    (account_obj) => account_obj.account_id === login_id
  );

const getCurrCtraderAccount = (ctrader_accounts_list, login_id) =>
  ctrader_accounts_list.find(
    (account_obj) => account_obj.account_id === login_id
  );
...
const Content = ({ currency_icon, loginid, title, value }) => (
  <div className="closing-account-error__container">
    ...
    <Text
      className="closing-account-error__details"
      color="prominent"
      size="xs"
      line_height="s"
      align="right"
  ...
  if (details.open_positions) {
    Object.keys(details.open_positions).forEach((login_id) => {
      ...
      if (deriv_account) {
        ...
        const dxtrade_account = getCurrDxtradeAccount(
          dxtrade_accounts_list,
          login_id
        );
        if (dxtrade_account) {
        ...
        const ctrader_account = getCurrCtraderAccount(
          ctrader_accounts_list,
          login_id
        );
        if (ctrader_account) {
    ...
    <React.Fragment>
      <ThemedScrollbars autohide={false} width="43rem">
        {!!deriv_open_positions.length && (
          <Wrapper
            title={localize(
              'Please close your positions in the following Deriv account(s):'
            )}
            desc={undefined}
          >
        ...
        {!!mt5_open_positions.length && (
          <Wrapper
            title={localize(
              'Please close your positions in the following Deriv MT5 account(s):'
            )}
            desc={undefined}
          >
        ...
        {!!mt5_balance.length && (
          <Wrapper
            title={localize(
              'Please withdraw your funds from the following Deriv MT5 account(s):'
            )}
            desc={undefined}
          >
        ...
        {!!dxtrade_open_positions.length && (
          <Wrapper
            title={localize(
              'Please close your positions in the following Deriv X account(s):'
            )}
            desc={undefined}
          >
        ...
        {!!dxtrade_balance.length && (
          <Wrapper
            title={localize(
              'Please withdraw your funds from the following Deriv X account(s):'
            )}
            desc={undefined}
          >
