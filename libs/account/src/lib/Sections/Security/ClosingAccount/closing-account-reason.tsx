...
import AccountHasPendingConditions from './account-has-balance.js';
import ClosingAccountReasonFrom from './closing-account-reason-form.js';

const preparingReason = (values) => {
  let selected_reasons = selectedReasons(values)
    .map((val) => val[0])
    .toString();
  const is_other_trading_platform__has_value =
    !!values.other_trading_platforms.length;
...
const selectedReasons = (values) => {
  return Object.entries(values).filter(
    ([key, value]) =>
      !['other_trading_platforms', 'do_to_improve'].includes(key) && value
  );
};
    ...
    <div className="account-closure-warning-modal">
      ...
      <div className="account-closure-warning-modal__content-wrapper">
        <Text
          as="p"
          align="center"
          className="account-closure-warning-modal__content"
        >
          {localize(
...
const GeneralErrorContent = ({ message, onClick }) => (
  <>
    ...
    <div>
      <Button
        className="closing-account-error__button"
        primary
        onClick={onClick}
      >
        {localize('OK')}
...
const ClosingAccountReason = observer(({ onBackClick }) => {
  ...
  const [which_modal_should_render, setWhichModalShouldRender] =
    React.useState();
  ...
  const [remaining_characters, setRemainingCharacters] =
    React.useState(character_limit_no);
  const [total_accumulated_characters, setTotalAccumulatedCharacters] =
    React.useState(0);
  ...
  const validateFields = (values) => {
    ...
    const text_inputs_length = (
      values.other_trading_platforms + values.do_to_improve
    ).length;
    ...
    if (selected_reason_count) {
      ...
      if (!/^[ a-zA-Z0-9.,'-\s]*$/.test(final_value)) {
        // @ts-expect-error fix this
        error.characters_limits = localize(
          "Must be numbers, letters, and special characters . , ' -"
        );
      }
    } else {
      // @ts-expect-error fix this
  ...
  const handleInputPaste = (e) => {
    ...
    if (
      remaining_characters <= 0 ||
      clipboardData.length > remaining_characters
    ) {
      e.preventDefault();
    }
  };
  ...
  const getModalTitle = () => {
    if (which_modal_should_render === 'error_modal')
      return localize('An error occurred');
    if (which_modal_should_render === 'inaccessible_modal')
      return localize('Inaccessible MT5 account(s)');
    return which_modal_should_render !== 'warning_modal'
  ...
  const startDeactivating = async () => {
    ...
    if (account_closure_response.account_closure === 1) {
      ...
      } = account_closure_response.error;
  ...
  return (
    ...
    >
      <Text
        weight="bold"
        size="xs"
        className="closing-account-reasons__title"
        as="p"
      >
        {localize(
          'Please tell us why you’re leaving. (Select up to {{ allowed_reasons }} reasons.)',
          {
