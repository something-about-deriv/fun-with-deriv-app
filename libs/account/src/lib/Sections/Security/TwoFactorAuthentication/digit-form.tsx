...
const DigitForm = ({
  ...
  const [is_ready_for_verification, setReadyForVerification] =
  ...
  const validateFields = async (values) => {
    ...
    if (!digit_code) {
      ...
      return {
        digit_code: localize('Length of digit code must be 6 characters.'),
      };
  ...
  const handleSubmit = async (values, { resetForm }) => {
    if (!enable_response.error) {
      const is_enabled_response = !!getPropertyValue(enable_response, [
        'account_security',
        'totp',
        'is_enabled',
      ]);
