...
const SubmittedPage = ({ platform, routeBackInApp }: TSubmittedPage) => {
  ...
  return (
    <IconMessageContent
      ...
      message={localize('Financial assessment submitted successfully')}
      text={localize(
        'Let’s continue with providing proofs of address and identity.'
      )}
      icon={<Icon icon="IcSuccess" width={96} height={90} />}
    >
      <div className="account-management-flex-wrapper account-management-submit-success">
...
const FinancialAssessment = observer(() => {
  ...
  const [is_confirmation_visible, setIsConfirmationVisible] =
    React.useState(false);
  const [has_trading_experience, setHasTradingExperience] =
    React.useState(false);
  const [is_form_visible, setIsFormVisible] = React.useState(true);
  const [api_initial_load_error, setApiInitialLoadError] =
  ...
  const onSubmit = async (
    ...
    if (data.error) {
      ...
      WS.authorized.storage
        .getFinancialAssessment()
        .then((res_data: GetFinancialAssessmentResponse) => {
          if (res_data?.get_financial_assessment)
  ...
  const onClickSubmit = (handleSubmit: () => void) => {
    const is_confirmation_needed =
      has_trading_experience && is_trading_experience_incomplete;
  ...
  const getScrollOffset = () => {
    if (is_mf) {
      ...
      return is_financial_information_incomplete && !is_submit_success
        ? '165px'
        : '160px';
  ...
  if (isMobile() && is_authentication_needed && !is_mf && is_submit_success)
    return (
      ...
      <SubmittedPage platform={platform} routeBackInApp={routeBackInApp} />
    );
                  ...
                  <fieldset className="account-form__fieldset">
                    ...
                    <MobileWrapper>
                      <SelectNative
                        ...
                        // @ts-expect-error TODO: fix this
                        error={
                          touched.occupation ? errors.occupation : undefined
                        }
                        onChange={(e) => {
                          setFieldValue(
                            'occupation',
                  ...
                  <fieldset className="account-form__fieldset">
                    ...
                    <MobileWrapper>
                      <SelectNative
                        ...
                        // @ts-expect-error TODO: fix this
                        error={
                          touched.net_income ? errors.net_income : undefined
                        }
                        onChange={(e) => {
