...
import POISubmission from './proof-of-identity-submission.js';
...
const ProofOfIdentityContainer = observer(
  ({
    ...
    const [has_require_submission, setHasRequireSubmission] =
    ...
    const { app_routing_history, routeBackInApp, is_language_changing } =
      common;
    const { refreshNotifications } = notifications;

    const from_platform = getPlatformRedirect(app_routing_history);

    const should_show_redirect_btn = Object.keys(platforms).includes(
      from_platform?.ref
    );

    const routeBackTo = (redirect_route) =>
      routeBackInApp(history, [redirect_route]);
    ...
    if (
      ...
      (identity_status === identity_status_codes.verified &&
        identity_status !== identity_last_attempt.status)
    ) {
