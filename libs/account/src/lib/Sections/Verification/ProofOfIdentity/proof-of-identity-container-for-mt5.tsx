...
import POISubmissionForMT5 from './proof-of-identity-submission-for-mt5.js';
import { service_code } from './proof-of-identity-utils';
import { populateVerificationStatus } from '../Helpers/verification';

const ProofOfIdentityContainerForMt5 = observer(
  ({ onStateChange, citizen_data }: { onStateChange; citizen_data }) => {
    ...
    const { account_status, fetchResidenceList, is_switching, is_virtual } =
      client;
    ...
    if (is_status_loading || is_switching) {
      ...
      return (
        <IconWithMessage
          icon="IcPoaVerified"
          message={localize(
            'Switch to your real account to submit your documents'
          )}
          has_button
        />
      );
    ...
    const { idv, identity_last_attempt, is_idv_disallowed, onfido } =
      verification_status;

    const poi_resubmission_cases = ['rejected', 'suspected', 'expired'];

    const has_idv_error =
      identity_last_attempt?.service &&
      service_code.idv &&
      poi_resubmission_cases.includes(idv.status);
