...
const POISubmission = observer(
  ({
    ...
    const handleSelectionNext = () => {
      if (Object.keys(selected_country).length) {
        ...
        const is_idv_supported =
          selected_country.identity.services.idv.is_country_supported;
        const is_onfido_supported =
          selected_country.identity.services.onfido.is_country_supported &&
          selected_country.value !== 'ng';
    ...
    const getCountryFromResidence = React.useCallback(
      (country_code) =>
        residence_list.find((residence) => residence.value === country_code),
      [residence_list]
    );

    const needs_resubmission = has_require_submission || allow_poi_resubmission;

    const mismatch_status = formatIDVError(
      idv.last_rejected,
      idv.status,
      is_high_risk
    );
      ...
      (identity_last_attempt) => {
        ...
        switch (service) {
          ...
          case service_code.onfido: {
            if (
              Number(idv.submissions_left) > 0 ||
              Number(onfido.submissions_left) > 0
            ) {
              setSubmissionStatus(submission_status_code.selecting);
            } else {
              setSubmissionService(service_code.manual);
      ...
      case submission_status_code.submitting: {
        switch (submission_service) {
          ...
          case service_code.onfido: {
            ...
            const doc_obj =
              selected_country.identity.services.onfido.documents_supported;
            const documents_supported = Object.keys(doc_obj).map(
              (d) => doc_obj[d].display_name
            );
