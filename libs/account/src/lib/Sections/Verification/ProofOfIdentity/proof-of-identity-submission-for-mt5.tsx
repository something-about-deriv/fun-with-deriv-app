...
const POISubmissionForMT5 = observer(
  ({
    ...
    React.useEffect(() => {
      if (citizen_data) {
        const {
          submissions_left: idv_submissions_left,
          last_rejected,
          status,
        } = idv;
        const { submissions_left: onfido_submissions_left } = onfido;
        const is_idv_supported = isVerificationServiceSupported(
          residence_list,
          account_settings,
          'idv'
        );
        const is_onfido_supported = isVerificationServiceSupported(
          residence_list,
          account_settings,
          'onfido'
        );
    ...
    if (submission_status === submission_status_code.submitting) {
      switch (submission_service) {
        ...
        case service_code.onfido: {
          ...
          const doc_obj =
            citizen_data.identity.services.onfido.documents_supported;
