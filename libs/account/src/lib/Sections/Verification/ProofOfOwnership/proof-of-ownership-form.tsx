...
import { isMobile, readFiles, WS, DOCUMENT_TYPES } from '@deriv/shared';
...
const ProofOfOwnershipForm = ({
  ...
  const grouped_payment_method_data_keys = Object.keys(
    grouped_payment_method_data
  );
  ...
  const validateFields = (values) => {
    ...
    Object.keys(cards)?.forEach?.((card_key) => {
      ...
      item_keys?.forEach?.((item_key) => {
        if (!has_errors) {
          ...
          // @ts-expect-error fix this
          errors.data[card_key][item_key] =
            // @ts-expect-error fix this
            errors.data?.[card_key]?.[item_key] ?? {};
          const payment_method = items?.[item_key];
          const payment_method_identifier =
            payment_method?.payment_method_identifier?.trim();
          const is_payment_method_identifier_provided =
            payment_method?.is_generic_pm ||
            payment_method_identifier?.length > 0;
          const is_credit_or_debit_card =
            payment_method?.identifier_type === IDENTIFIER_TYPES.card_number;
          total_documents_uploaded =
            payment_method?.files?.filter(Boolean)?.length ?? 0;
          if (is_payment_method_identifier_provided) {
            ...
            (!is_payment_method_identifier_provided &&
              total_documents_uploaded ===
                payment_method?.documents_required * 0.5)
          ) {
          ...
          payment_method?.files?.forEach((file, i) => {
            // @ts-expect-error fix this
            errors.data[card_key][item_key].files =
              // @ts-expect-error fix this
              errors?.data?.[card_key]?.[item_key]?.files ?? [];
            if (
              file?.type &&
              !/(image|application)\/(jpe?g|pdf|png)$/.test(file?.type)
            ) {
              // @ts-expect-error fix this
          ...
          if (
            ...
            ((payment_method_identifier?.length !== 0 &&
              (payment_method_identifier?.length !== 16 ||
                payment_method_identifier?.length > 19) &&
              !VALIDATIONS.is_formated_card_number(
          ...
          if (!payment_method_identifier && total_documents_uploaded === 0) {
            ...
            if (
              // @ts-expect-error fix this
              (form_ref.current?.values?.data?.[card_key]?.filter?.(Boolean)
                ?.length || 0) === 0
            ) {
              // @ts-expect-error fix this
          ...
          if (
            // @ts-expect-error fix this
            Object.keys(errors?.data?.[card_key]?.[item_key] || {}).length === 0
          ) {
            // @ts-expect-error fix this
          ...
          has_errors =
            has_errors ||
            // @ts-expect-error fix this
            errors?.data?.[card_key]?.[
              item_key
            ]?.payment_method_identifier?.trim?.()?.length > 0 ||
            // @ts-expect-error fix this
      ...
      if (
        // @ts-expect-error fix this
        (form_ref.current?.values?.data?.[card_key]?.filter(Boolean)?.length ||
          0) === 0
      ) {
        // @ts-expect-error fix this
        delete form_ref.current?.values?.data?.[card_key];
    ...
    has_errors =
      has_errors ||
      // @ts-expect-error fix this
      (form_ref.current?.values?.data?.filter(Boolean).length || 0) === 0;
    if (!has_errors) {
      errors = {};
    }
    return errors;
  ...
  const updateErrors = async (index, item_index, sub_index) => {
    ...
    if (typeof errors.data[index] === 'object') {
      ...
      // @ts-expect-error fix this
      const has_other_errors = errors?.data[index]?.[item_index]?.files?.some(
        (error) => error !== null
      );
      if (!has_other_errors) {
        // @ts-expect-error fix this
      ...
      Object.keys(form_values).forEach((card_key) => {
        Object.keys(form_values[card_key]).forEach(async (card_item_key) => {
          ...
          if (payment_method_details.files.length > 0) {
            const processed_files = await readFiles(
              payment_method_details.files,
              fileReadErrorMessage,
              {
                documentType: DOCUMENT_TYPES.proof_of_ownership,
                proof_of_ownership: {
                  details: {
                    email: client_email,
                    payment_identifier:
                      payment_method_details.payment_method_identifier,
                  },
              ...
              if (response.warning) {
                if (response.warning.trim() === 'DuplicateUpload') {
                  ...
                  form_errors.data[card_key][card_item_key].files =
                    form_errors?.data?.[card_key]?.[card_item_key]?.files ?? [];
                  form_errors.data[card_key][card_item_key].files[sub_index] =
                    response.message; // Document already uploaded
                  // @ts-expect-error fix this
    ...
    >
      {({ values, errors, setFieldValue, handleSubmit, isValid, dirty }) => (
        ...
        >
          ...
          >
            <FormSubHeader
              title={localize('Please upload the following document(s).')}
            />
            <FormBodySection>
              <fieldset>
                {grouped_payment_method_data_keys?.map(
                  (grouped_payment_method_data_key, index) => (
                    <div
