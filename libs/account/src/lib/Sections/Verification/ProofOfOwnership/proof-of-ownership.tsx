...
import {
  ...
  POORejetced,
  POOSubmitted,
} from 'Components/poo/statuses';
import { Loading } from '@deriv/components';
import { POO_STATUSES } from './constants/constants.js';
...
export const ProofOfOwnership = observer(() => {
  ...
  const citizen =
    client?.account_settings?.citizen || client?.account_settings?.country_code;
  ...
  const grouped_payment_method_data = React.useMemo(() => {
    ...
    cards?.forEach((card) => {
      const card_details =
        payment_methods_config[card.payment_method.toLowerCase()] ||
        payment_methods_config.other;
      if (groups[card?.payment_method?.toLowerCase()]) {
        ...
        groups[card?.payment_method?.toLowerCase()] = {
          ...
          icon: is_dark_mode
            ? card_details?.icon_dark
            : card_details?.icon_light,
