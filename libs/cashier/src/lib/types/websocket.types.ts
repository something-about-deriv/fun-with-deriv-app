...
import type { TSocketEndpointNames, TSocketResponse } from '@deriv/api';
...
type TWebSocketCall = {
  ...
  paymentAgentList: (
    residence: string,
    currency: string
  ) => Promise<PaymentAgentListResponse>;
  paymentAgentWithdraw: (
    request: TPaymentAgentWithdrawRequest
  ) => Promise<PaymentAgentWithdrawResponse>;
};
...
export type TWebSocket = {
  ...
  send: (
    ...
    error: TServerError;
    exchange_rates: { rates: { [k: string]: string } };
  }>;
  ...
  wait: <T extends TSocketEndpointNames>(
    value: T
