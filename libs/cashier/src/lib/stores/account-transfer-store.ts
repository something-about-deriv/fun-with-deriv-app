...
const hasTransferNotAllowedLoginid = (loginid?: string) =>
  loginid?.startsWith('MX');
...
export default class AccountTransferStore {
  ...
  get is_transfer_locked() {
    ...
    return (
      need_financial_assessment && this.error.is_ask_financial_risk_approval
    );
  }
  ...
  async onMountAccountTransfer() {
    ...
    const { onMountCommon, setLoading, setOnRemount } =
    ...
    const has_updated_account_balance =
      ...
      Object.keys(active_accounts).find(
        (account) =>
          !active_accounts[Number(account)].is_virtual &&
          active_accounts[Number(account)].balance
      );
    ...
    if (is_logged_in) {
      const transfer_between_accounts =
        await this.WS.authorized.transferBetweenAccounts();
      ...
      await this.sortAccountsTransfer(
      ...
      if (this.accounts_list?.length > 0) {
        const cfd_transfer_to_login_id = sessionStorage.getItem(
          'cfd_transfer_to_login_id'
        );
        sessionStorage.removeItem('cfd_transfer_to_login_id');
        const obj_values = this.accounts_list.find(
        ...
        if (obj_values) {
          if (hasTransferNotAllowedLoginid(obj_values.value)) {
            ...
            obj_values.error = AccountTransferGetSelectedError(
              obj_values.value
            );
          }
  ...
  setTransferLimit() {
    const transfer_limits = [
      ...
      },
      {
        ...
        is_transfer:
          this.selected_from.is_dxtrade || this.selected_to.is_dxtrade,
      },
      {
        limits: 'limits_ctrader',
        is_transfer:
          this.selected_from.is_ctrader || this.selected_to.is_ctrader,
      },
    ];

    const limits_key =
      transfer_limits.find((option) => option.is_transfer)?.limits || 'limits';
    ...
    this.transfer_limit = {
      max:
        !transfer_limit?.max ||
        (Number(balance) >= (transfer_limit?.min || 0) &&
          Number(balance) <= transfer_limit?.max)
          ? balance
          : transfer_limit?.max.toFixed(decimal_places),
      min: transfer_limit?.min
        ? (+transfer_limit?.min).toFixed(decimal_places)
        : '',
    };
  }
  ...
  async sortAccountsTransfer(
    ...
    const transfer_between_accounts =
      response_accounts || (await this.WS.authorized.transferBetweenAccounts());
    ...
    if (
      !is_from_derivgo &&
      transfer_between_accounts &&
      Array.isArray(transfer_between_accounts.accounts)
    ) {
      transfer_between_accounts.accounts =
        transfer_between_accounts.accounts || [];
    }

    const mt5_login_list = (await this.WS.storage.mt5LoginList())
      ?.mt5_login_list;
    // TODO: move `tradingPlatformAccountsList` to deriv-api to use storage
    const dxtrade_accounts_list = (
      await this.WS.tradingPlatformAccountsList(CFD_PLATFORMS.DXTRADE)
    )?.trading_platform_accounts;

    const ctrader_accounts_list = (
      await this.WS.tradingPlatformAccountsList(CFD_PLATFORMS.CTRADER)
    )?.trading_platform_accounts;
    ...
    const accounts = transfer_between_accounts.accounts?.map((account) => {
      if (
        ...
        Array.isArray(mt5_login_list) &&
        mt5_login_list.length
      ) {
        ...
        const found_account = mt5_login_list.find(
          (acc) => acc.login === account.loginid
        );
        ...
        return {
          ...account,
          ...found_account,
          account_type: CFD_PLATFORMS.MT5,
        };
      }
      if (
        ...
        const found_account = ctrader_accounts_list.find(
          (acc) => acc.account_id === account.loginid
        );
        ...
        return {
          ...account,
          ...found_account,
          account_type: CFD_PLATFORMS.CTRADER,
        };
      }
      if (
        ...
        // since in cashier all these are real accounts, the mt5 account type is what we want to keep
        const found_account = dxtrade_accounts_list.find(
          (acc) => acc.account_id === account.loginid
        );
        ...
        return {
          ...account,
          ...found_account,
          account_type: CFD_PLATFORMS.DXTRADE,
        };
      }
      return account;
    });
    ...
    const is_from_outside_cashier = !location.pathname.startsWith(
      routes.cashier
    );
    ...
    accounts?.forEach((account: TTransferAccount) => {
      ...
      const cfd_text_display =
        cfd_platforms[account.account_type as keyof typeof cfd_platforms]?.name;
      ...
      if (account.loginid === this.root_store.client.loginid) {
        ...
        const account_id =
          this.root_store.traders_hub?.selected_account.account_id;
        ...
        if (
          ...
          [account_id, login].includes(account.loginid)
        ) {
          this.setSelectedTo(obj_values);
        }
      }
  ...
  onChangeTransferFrom({ target }: { target: { value: string } }) {
    ...
    const selected_from = accounts.find(
      (account) => account.value === target.value
    );
    ...
    if (selected_from?.value === this.selected_to.value) {
      ...
      this.onChangeTransferTo({
        target: { value: this.root_store.client.loginid },
      });
    }

    if (selected_from && hasTransferNotAllowedLoginid(selected_from?.value)) {
      selected_from.error = AccountTransferGetSelectedError(
        selected_from?.value,
        true
      );
    }
  ...
  onChangeTransferTo({ target }: { target: { value: string | undefined } }) {
    ...
    if (hasTransferNotAllowedLoginid(this.selected_to.value)) {
      this.selected_to.error = AccountTransferGetSelectedError(
  ...
  requestTransferBetweenAccounts = async ({ amount }: { amount: number }) => {
    ...
    if (transfer_between_accounts.error) {
      ...
      if (
        transfer_between_accounts.error.code === 'Fiat2CryptoTransferOverLimit'
      ) {
        const account_status_response =
          await this.WS.authorized.getAccountStatus();
      ...
      this.setReceiptTransfer({
        amount: formatMoney(currency || '', amount, true),
      });
  ...
  setTransferPercentageSelectorResult(
    ...
    const { crypto_fiat_converter, general_store } =
  ...
  validateTransferFromAmount() {
    const converter_from_amount =
      this.root_store.modules.cashier?.crypto_fiat_converter
        .converter_from_amount;
    const setConverterFromError =
      this.root_store.modules.cashier?.crypto_fiat_converter
    ...
    if (!converter_from_amount) {
      ...
      if (!is_ok) {
        ...
        Number(this.selected_from.balance) < Number(converter_from_amount)
      ) {
  ...
  validateTransferToAmount() {
    const converter_to_amount =
      this.root_store.modules.cashier?.crypto_fiat_converter
        .converter_to_amount;
    const setConverterToError =
      this.root_store.modules.cashier?.crypto_fiat_converter
        .setConverterToError;
