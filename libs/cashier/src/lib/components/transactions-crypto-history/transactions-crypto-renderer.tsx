...
import type { TSocketResponse } from '@deriv/api';

type TTransactionsCryptoRendererProps = {
  row: NonNullable<
    TSocketResponse<'cashier_payments'>['cashier_payments']
  >['crypto'][number];
  onTooltipClick: VoidFunction;
};

const TransactionsCryptoRenderer = observer(
  ({ row: crypto, onTooltipClick }: TTransactionsCryptoRendererProps) => {
    const { client } = useStore();
    const { transaction_history } = useCashierStore();
    const {
      cancelCryptoTransaction,
      showTransactionsCryptoCancelModal,
      showTransactionsCryptoStatusModal,
    } = transaction_history;
    ...
    const formatted_address_hash = address_hash
      ? `${address_hash.substring(0, 4)}....${address_hash.substring(
    ...
    const formatted_submit_time =
      epochToMoment(submit_date).format('HH:mm:ss [GMT]');
    ...
    const [is_transaction_clicked, setTransactionClicked] =
    ...
    const onClickStatus = () => {
      if (status)
        showTransactionsCryptoStatusModal(status.description, status.name);
    };
          ...
          <Table.Row className="transactions-crypto-history__table-row">
            <Table.Cell className="transactions-crypto-history__table-cell">
              ...
              <Text
                ...
                size="xs"
                weight="bold"
                className="transactions-crypto-history__table-type"
              >
            ...
            <Table.Cell className="transactions-crypto-history__table-amount">
              <Text
                ...
                weight="bold"
                color={
                  status.renderer === 'successful' ? 'profit-success' : 'red'
                }
              >
                <Money
                  currency={currency}
                ...
                <>
                  ...
                  {is_third_party_transaction && (
                    <Popover
                      ...
                      message={localize(
                        'The details of this transaction is available on CoinsPaid.'
                      )}
                    >
                      <Icon
                        icon="IcHelpCentre"
            ...
            <Table.Cell className="transactions-crypto-history__table-action">
              {is_valid_to_cancel === 1 && (
                <div
                  ...
                  data-testid="dt_transactions_crypto_history_table_button"
                >
                  <Popover
                    alignment="left"
                    className="transactions-crypto-history__table-popover"
