...
const Cashier = observer(
  ({ history, location, routes: routes_config }: TCashierProps) => {
    ...
    const getHeaderTitle = useMemo(() => {
      if (
        !is_mobile ||
        (is_default_route && (is_loading || is_cashier_onboarding))
      )
    ...
    useEffect(() => {
      if (
        !is_onramp_visible &&
        history.location.pathname === routes.cashier_onramp
      ) {
        history.push(routes.cashier_deposit);
      }
    ...
    useEffect(() => {
      if (
        ...
        !is_p2p_enabled &&
        history.location.pathname.startsWith(routes.cashier_p2p)
      ) {
          ...
          if (is_virtual) {
            ...
            error.setErrorMessage({
              ...
              message:
                currency !== 'USD' && is_svg
                  ? localize(
                      'Deriv P2P is currently unavailable in this currency.'
                    )
                  : localize(
                      'Deriv P2P is currently unavailable in your country.'
                    ),
            });
          }
        }
    ...
    return (
      <FadeWrapper
        ...
        className="cashier__page-wrapper"
        keyname="cashier__page-wrapper"
      >
        ...
        <div className="cashier">
          ...
          >
            ...
            <MobileWrapper>
              <Div100vhContainer
                className="cashier__wrapper--is-mobile"
                height_offset="80px"
              >
