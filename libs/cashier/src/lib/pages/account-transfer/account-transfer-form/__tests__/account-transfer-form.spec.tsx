...
describe('<AccountTransferForm />', () => {
  ...
  it('component should be rendered', () => {
    ...
    expect(
      screen.getByTestId('dt_account_transfer_form_wrapper')
    ).toBeInTheDocument();
  ...
  it('should show <Form /> component if account_list.length > 0', () => {
    ...
    expect(
      screen.getByTestId('dt_account_transfer_form_drop_down_wrapper')
    ).toBeInTheDocument();
    expect(
      screen.getByTestId('dt_account_transfer_form_drop_down')
    ).toBeInTheDocument();
    expect(
      screen.getByTestId('dt_account_transfer_form_to_dropdown')
    ).toBeInTheDocument();
    expect(
      screen.getByTestId('dt_account_transfer_form_submit')
    ).toBeInTheDocument();
    expect(
  ...
  it('should show an error if amount is not provided', async () => {
    ...
    fireEvent.change(screen.getByTestId('dt_account_transfer_form_input'), {
      target: { value: '1' },
    });
    fireEvent.change(screen.getByTestId('dt_account_transfer_form_input'), {
      target: { value: '' },
  ...
  it('should not allow to do transfer if accounts from and to are same', () => {
    ...
    mockRootStore.modules.cashier.account_transfer.accounts_list[0].is_mt =
      true;
  ...
  it('should show input if same currency', () => {
    ...
    expect(
      screen.getByTestId('dt_account_transfer_form_input')
    ).toBeInTheDocument();
  });
  ...
  it('should show proper hint about mt5 remained transfers', () => {
    ...
    expect(
      screen.getByText('You have 1 transfer remaining for today.')
    ).toBeInTheDocument();
  });
  ...
  it('should show proper hint about dxtrade remained transfers', () => {
    ...
    mockRootStore.modules.cashier.account_transfer.selected_from.is_dxtrade =
      true;
    mockRootStore.modules.cashier.account_transfer.selected_from.currency =
      'USD';
    mockRootStore.modules.cashier.account_transfer.selected_to.is_dxtrade =
      true;
  ...
  it('should display "no new positions can be opened" when transferring amount to a migrated svg account with position', () => {
    mockRootStore.modules.cashier.account_transfer.selected_to.status =
      'migrated_with_position';
    renderAccountTransferForm();

    expect(
      screen.getByText(
        /You can no longer open new positions with this account./i
      )
    ).toBeInTheDocument();
    expect(
      screen.queryByText(/You have 0 transfer remaining for today./i)
    ...
    describe('from_dropdown', () => {
      it('should check for USD icon when USD is selected in from_dropdown', () => {
        ...
        mockRootStore.modules.cashier.account_transfer.selected_from =
          currency_usd_account;
        mockRootStore.modules.cashier.account_transfer.setTransferPercentageSelectorResult =
          jest.fn().mockReturnValue(10.0);

        renderAccountTransferForm();
        expect(
          screen.getByTestId('dt_account_platform_icon_currency_usd')
        ).toBeInTheDocument();
      });
      ...
      it('should check for DerivX icon when DerivX is selected in from_dropdown', () => {
        ...
        renderAccountTransferForm();
        expect(
          screen.getByTestId('dt_account_platform_icon_IcRebrandingDeriv X')
        ).toBeInTheDocument();
      });
    });
  });
