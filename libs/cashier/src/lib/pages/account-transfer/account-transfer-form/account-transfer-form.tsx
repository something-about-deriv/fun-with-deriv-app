...
const AccountOption = ({ account, idx }: TAccountsList) => {
  const is_cfd_account =
    account.is_dxtrade || account.is_ctrader || account.is_mt;
    ...
    <React.Fragment key={idx}>
      ...
      <div className="account-transfer-form__currency-wrapper">
        <Text
          size="xxs"
          line_height="xs"
          styles={{ color: 'prominent', fontWeight: 'inherit' }}
        >
...
const AccountTransferForm = observer(
  ({
    ...
    const [arrow_icon_direction, setArrowIconDirection] = React.useState<
      'right' | 'left'
    >('right');
    const {
      ui,
    ...
    const { account_transfer, crypto_fiat_converter, general_store } =
      useCashierStore();
    ...
    const is_ctrader_transfer =
      selected_to.is_ctrader || selected_from.is_ctrader;
    const is_dxtrade_transfer =
      selected_to.is_dxtrade || selected_from.is_dxtrade;
    ...
    const validateAmount = (amount: string) => {
      ...
      if (
        typeof selected_from.balance !== 'undefined' &&
        Number(selected_from.balance) < Number(amount)
      )
    ...
    const shouldShowTransferButton = (amount: string) => {
      return selected_from.currency === selected_to.currency
        ? !amount
        : !converter_from_amount;
    };
    ...
    const getAccounts = (
      type: string,
      { is_mt, is_ctrader, is_dxtrade }: TAccount
    ) => {
      if (type === 'from') {
    ...
    React.useEffect(() => {
      if (selected_from?.currency && selected_to?.currency) {
        ...
        const base_currency = is_arrow_right
          ? selected_from.currency
          : selected_to.currency;
        const target_currency = is_arrow_right
          ? selected_to.currency
          : selected_from.currency;
    ...
    React.useEffect(() => {
      ...
      accounts_list.forEach((account, idx) => {
        ...
        const is_cfd_account =
          account.is_mt || account.is_ctrader || account.is_dxtrade;
        ...
        if (!is_selected_from) {
          ...
          const is_selected_from_ctrader =
            selected_from.is_ctrader && account.is_ctrader;
          const is_selected_from_dxtrade =
            selected_from.is_dxtrade && account.is_dxtrade;
          ...
          const is_disabled =
            is_selected_from_mt ||
            is_selected_from_ctrader ||
            is_selected_from_dxtrade;
      ...
      setFromAccounts({
        ...(mt_accounts_from.length && {
          [localize('Deriv MT5 accounts')]: mt_accounts_from,
        }),
        ...(ctrader_accounts_from.length && {
          [localize('Deriv cTrader accounts')]: ctrader_accounts_from,
        }),
        ...(dxtrade_accounts_from.length && {
          [localize('{{platform_name_dxtrade}} accounts', {
            platform_name_dxtrade,
          })]: dxtrade_accounts_from,
        }),
        ...(accounts_from.length && {
      ...
      setToAccounts({
        ...(mt_accounts_to.length && {
          [localize('Deriv MT5 accounts')]: mt_accounts_to,
        }),
        ...(ctrader_accounts_to.length && {
          [localize('Deriv cTrader accounts')]: ctrader_accounts_to,
        }),
    ...
    React.useEffect(() => {
      ...
      if (is_migration_status_present) {
        hint_text = (
          <Localize i18n_default_text="You can no longer open new positions with this account." />
        );
      } else {
        const transfer_text =
          remaining_transfers > 1 ? 'transfers' : 'transfer';
        hint_text = (
          <Localize
    ...
    const Mt5RestrictedMsg = () => (
      <Localize
        ...
        components={[
          <Link key={0} to={routes.proof_of_address} className="link dark" />,
        ]}
      />
    );
    ...
    const getMt5Error = () => {
      if (is_mt5_restricted) {
        return authentication_status?.document_status === 'pending' ? (
          poa_pending_msg
        ) : (
          <Mt5RestrictedMsg />
        );
    ...
    const NotesLink = () => {
      return (
        <div
          className="account-transfer-form__button-link"
      ...
      >
        ...
        <Formik
          ...
          onSubmit={() => {
            requestTransferBetweenAccounts({
              amount: account_transfer_amount
                ? Number(account_transfer_amount)
                : 0,
            });
          }}
          ...
          {({
            ...
            <React.Fragment>
              {isSubmitting || accounts_list.length === 0 ? (
                <div
                  className="cashier__loader-wrapper"
                  data-testid="dt_cashier_loader_wrapper"
                >
