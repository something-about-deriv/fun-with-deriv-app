...
import { TModifiedTransaction } from '@deriv/hooks';
...
const Status = ({ last_transaction }: TStatusProps) => {
  ...
  React.useEffect(() => {
    if (last_transaction) {
      const { status_color, status_name } =
  ...
  return (
    <Text
      ...
      weight="bold"
      align="center"
      className="withdrawal-crypto-receipt__status"
    >
      <Icon
...
const WithdrawalCryptoReceipt = observer(() => {
  ...
  const { account_transfer, general_store, transaction_history, withdraw } =
    useCashierStore();
  ...
  return (
    <div className="cashier__wrapper">
      ...
      <div className="withdrawal-crypto-receipt__detail">
        ...
        <Icon
          className="withdrawal-crypto-receipt__icon"
          icon="IcArrowDown"
          size={30}
        />
        <WalletInformation
