...
type TCryptoTransactionItem = NonNullable<
...
export const getStatus = (
  ...
  const formatted_transaction_hash = transaction_hash
    ? `${transaction_hash.substring(0, 4)}....${transaction_hash.substring(
  ...
  const status_list = {
    ...
    withdrawal: {
          ...
          <Localize
            ...
            components={[
              <StaticUrl
                key={0}
                className="link"
                href="contact-us/?is_livechat_open=true"
              />,
            ]}
          />
        ),
      ...
      PROCESSING: {
        name: localize('In process'),
        description: localize(
          'We’re awaiting confirmation from the blockchain.'
        ),
        renderer: 'in-process',
        transaction_hash: formatted_transaction_hash,
      },
  ...
  ): status is keyof typeof status_list.deposit =>
    Object.keys(status_list.deposit).includes(status);
  const isWithdrawal = (
    status: TCryptoTransactionItem['status_code']
  ): status is keyof typeof status_list.withdrawal =>
  ...
  if (transaction_type === 'deposit') {
    if (status_code === 'CONFIRMED') {
      ...
      confirmation_label = confirmations
        ? `${confirmations}`
        : localize('Pending');
    }
  }
  ...
  if (transaction_type === 'deposit' && isDeposit(status_code)) {
    return {
      ...status_list[transaction_type][status_code],
      confirmation_label,
    };
