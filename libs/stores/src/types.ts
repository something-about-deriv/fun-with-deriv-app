...
import type { FeatureFlagsStore } from './lib/stores';
...
type TClientStore = {
  ...
  setLoginInformation: (
    client_accounts: { [k: string]: TActiveAccount },
    client_id: string
  ) => void;
  ...
  website_status: {
    mt5_status: TMt5StatusServer;
    dx_trade_status: TDXTraderStatusServerType;
  };
  email: string;
  ...
  isAccountOfTypeDisabled: (
    account: Record<string, DetailsOfEachMT5Loginid>
  ) => boolean;
...
type TUiStore = {
  ...
  setPromptHandler: (
    ...
    cb?:
      | (() => void)
      | ((route_to: RouteComponentProps['location'], action: string) => boolean)
  ) => void;
  setSubSectionIndex: (index: number) => void;
  shouldNavigateAfterChooseCrypto: (
    value: Omit<string, TRoutes> | TRoutes
  ) => void;
  ...
  populateSettingsExtensions: (
    menu_items: Array<TPopulateSettingsExtensionsMenuItem> | null
  ) => void;
...
type TOnChartBarrierChange =
  | null
  | ((barrier_1: string, barrier_2?: string) => void);
...
type TContractTradeStore = {
  ...
  clearAccumulatorBarriersData: (
    should_clear_contract_data_only?: boolean,
    should_clear_timeout?: boolean
  ) => void;
  clearError: () => void;
  ...
  updateAccumulatorBarriersData: ({
    ...
    TAccumulatorContractBarriersData & { underlying: string }
  >) => void;
...
type TTradersHubStore = {
  ...
  openFailedVerificationModal: (
    selected_account_type: Record<string, unknown> | string
  ) => void;
  ...
  setWalletModalActiveTab: (
    tab?: 'Deposit' | 'Withdraw' | 'Transfer' | 'Transactions'
  ) => void;
  ...
  startTrade: (
    platform?: TPlatform,
    existing_account?: DetailsOfEachMT5Loginid
  ) => void;
  getAccount: () => void;
